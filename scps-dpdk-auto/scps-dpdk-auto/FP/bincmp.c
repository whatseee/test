...
collectmatches (FILE * f1, FILE * f2, int32_t f1size)
{
  ...
  srchbuf = my_malloc ((size_t) (srchstrlen));
#ifdef DEBUG
		  ...
		  if (NULL == mz)
		    {
		      ...
		      mz = lastm = my_malloc (sizeof (struct _matches));
		      mz->prev = NULL;
		      if (NULL == mz)
			goto rerror;
		    }
		  else
		    {
		      ...
		      tmp = my_malloc (sizeof (struct _matches));
		      if (NULL == tmp)
			goto rerror;
		      tmp->prev = lastm;
		      lastm->next = tmp;
