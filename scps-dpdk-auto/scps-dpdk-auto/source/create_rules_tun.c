...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 sprintf (iptables_cmd, "iptables -A FORWARD --protocol tcp  -i %s -o %s -j MARK --set-mark 3", gw_ifs.aif_name, gw_ifs.bif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  //sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol tcp  -i %s -j ACCEPT",gw_ifs.aif_name);
  sprintf (iptables_cmd, "iptables -A FORWARD --protocol tcp  -i %s -o %s -j ACCEPT", gw_ifs.aif_name, gw_ifs.bif_name);
  //sprintf (iptables_cmd, "iptables -A FORWARD -t mangle --protocol tcp  -i %s -o %s -j RETURN", gw_ifs.aif_name, gw_ifs.aif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  //sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol tcp  -i %s -j MARK --set-mark 3", gw_ifs.bif_name);
sprintf (iptables_cmd, "iptables -A FORWARD --protocol tcp  -i %s -o %s -j MARK --set-mark 3", gw_ifs.bif_name, gw_ifs.aif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  //sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol tcp  -i %s -j ACCEPT",gw_ifs.bif_name);
  sprintf (iptables_cmd, "iptables -A FORWARD --protocol tcp  -i %s -o %s -j ACCEPT", gw_ifs.bif_name, gw_ifs.aif_name);
  //sprintf (iptables_cmd, "iptables -A FORWARD -t mangle --protocol tcp  -i %s -o %s -j RETURN", gw_ifs.bif_name, gw_ifs.bif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);
/*
  ...
  sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol udp  -i %s -j MARK --set-mark 1",gw_ifs.aif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol udp  -i %s -j ACCEPT",gw_ifs.aif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol udp  -i %s -j MARK --set-mark 2",gw_ifs.bif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);

  sprintf (iptables_cmd, "iptables -A PREROUTING -t mangle --protocol udp  -i %s -j ACCEPT",gw_ifs.bif_name);
  printf("iptables:: %s\n", iptables_cmd);
  SYSTEM (iptables_cmd);
*/
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
