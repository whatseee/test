...
#define GATEWAY_MEM_POOL_CLEAN_UP 1
	...
	printf("tp.c: entering main loop on lcore %u\n", lcore_id);
	...
	while (1)
...
#endif /* GATEWAY_SINGLE_THREAD */
		{
...
#endif /* SECURE_GATEWAY */
			{
...
#endif /* GATEWAY */
				{
					...
					{
						case SCPSTP:
							tp_Handler (&rqts_in, cc, tp, &already_freed);
							break;
						case SCPSCTP:
				...
				receives++;
				//printf("tp.c 292:already_freed=%d\n",already_freed);
				/* Free the buffer */
				if (!already_freed) 
				{
			...
			*/
			...
			if(alarm_handler_sign==1)
			{
				alarm_handler_new(1);
				alarm_handler_sign = 0;
				//printf("Gateway-->Function tp: sign = %d!\n", alarm_handler_sign);
				//fflush(stdout);
			}
...
#endif /* FAIRER_GATEWAY */
	  {
		if (s) 
		{
//if((s->VEGAS_ALPHA!=ALPHA)||(s->VEGAS_BETA!=BETA)||(s->VEGAS_GAMMA!=GAMMA))
			if((s->send_buff)&&(s->send_buff->max_size!=MAX_MBUFFS)
			||(s->receive_buff)&&(s->receive_buff->max_size!=MAX_MBUFFS) )
				printf("PAY ATTENTION TO THIS! s->sockid=%d!\n",s->sockid);
...
*/
	      ...
	      if ((s->send_buff) && (s->send_buff->snd_una) || (s->send_buff) &&(s->send_buff->send))
		{
	  ...
	  if ( (s->otimers[Persist]) && (!s->otimers[Persist]->set) && (s->send_buff) ){
	      if ((s->otimers[Rexmit])&&(!s->otimers[Rexmit]->set) && ( (s->send_buff->snd_una) ||
SEQ_GT (s->max_seqsent, s->snduna)) &&
         ...
         if ((s->otimers[TW]) && (!s->otimers[TW]->set) && (s->state ==tp_StateTIMEWT)) {
...
#ifdef GATEWAY_MEM_POOL_CLEAN_UP
	  {
		...
		continue;
		  ...
		  (current_s->rt_route->current_credit <= (int)(current_s->rt_route->MTU)) &&  
                  (rate_condition == GW_USING_RATE) ) {
