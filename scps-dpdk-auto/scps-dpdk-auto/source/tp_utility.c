...
//For getting current year-month-data-hour-minute-second
FILE *fp_log = NULL;
//FILE *fp_timer = NULL;
FILE *fp_cksum = NULL;            //YGX
...
#if 0
  fp_log = fopen("/ippep/snmp/tcpinfo.log", "a+");
  if(!fp_log)
     {
     printf("Error: can't open CTI log file for writing!!!\n");
     }
	/****************************YGX*******************************/
	
	fp_cksum = fopen("/checksum.log", "a+");
	if(!fp_cksum)
	{
		printf("Error: can't open checksum log file for writing!!!\n");
	}
	//BuildTable16( nCRC_CCITT );
#endif
	/****************************************************************************************************/
/*
  fp_timer = fopen("/ippep/tp/timer.log", "w+");
  if(!fp_timer)
    printf("Error: can't open CTI timer file for writing!!!\n");
*/
...
// move to the location after func init_default_routes()--------for shenqu
...
/*
	...
	{
		create_interface (htonl (gw_ifs.aif_local_ipaddr), htonl (gw_ifs.aif_remote_ipaddr));
	}
	else if (gw_ifs.bif_local_ipaddr) 
	{
		create_interface (htonl (gw_ifs.bif_local_ipaddr), htonl (gw_ifs.bif_remote_ipaddr));
	}
	else  
	{
...
//#endif
  ...
  timer_fn[Keep] = (procref) tp_TFKeep;
  timer_fn[TW] = (procref) tp_TFTimeWT;
  timer_fn[KA_L] = (procref) tp_TFKeepAlive_L;
  timer_fn[KA_W] = (procref) tp_TFKeepAlive_W;
...
printf("Gateway-->Function scps_Init: Delete the last line! scps_np_init\n");
  //scps_np_init ();
...
int tp_Common (tp_Socket * s)
{
	...
	for (temp = 0; temp < MAX_SCPS_SOCKET; temp++)
		...
		if (scheduler.sockets[temp].ptr == NULL)
		{
			...
			//printf("tp_Common: num_of_socket==%d !!!\n", scheduler.num_of_socket);
			break;
		}
    	}
...
#else /* DEBUG_SEQNUM */
  s->seqnum = s->snduna = s->seqsent = s->max_seqsent =
    s->old_seqsent = s->lastuwein = /*tp_now (4294967295 - 250000000 )*/10000;
...
//  printf ("%s Initial seqnum is %u\n", stringNow (), s->initial_seqnum);
  ...
  /* s->t_rxtcur = 0; */
  s->flags = tp_FlagSYN;
	s->th_off = temp = MBLEN - TP_MAX_HDR;  //88-60=28
	...
	s->sp_rqts.np_rqts.timestamp.ts_val[0] = s->sp_rqts.np_rqts.timestamp.ts_val[1] = 0;
...
SockAlloc_Failure:
  ...
  scheduler.num_of_socket--;
  //printf("tp_Common failed: num_of_socket==%d !!!\n", scheduler.num_of_socket);
      ...
      if ( (s == ds) )
		{
	   	...
	   	* anyway!
			...
			check_socket(31);
	  		...
	  		ds->state = tp_StateCLOSED;

         //my_sigprocmask (SIG_BLOCK, &alarmset, 0x0);
	  		/* Delete all our timers */
	  		for (i = 0; i < TIMER_COUNT; i++) 
			{
	    		delete_timer (ds->otimers[i], 0);
				ds->otimers[i] = NULL;
 	  		}
         //my_sigprocmask (SIG_UNBLOCK, &alarmset, 0x0);
...
*/
			...
			//There is something wrong with the code above! Modify it as below:
			if(ds->bbr)
		     {
		        my_free(ds->bbr);
		     }
  			...
  			if (ds->app_rbuff) 
			{
	  		...
	  		my_free (ds->receive_buff);
	  		my_free (ds->send_buff);
	  		my_free (ds->app_sbuff);
	  		my_free (ds->app_rbuff);
	  		my_free (ds->Out_Seq);
	  		...
	  		/* Prevent use from trying to my_free a phantom cluster */
	  		if (ds->scratch_buff)
			{
         	ds->scratch_buff->m_ext.offset = 0;
				ds->scratch_buff->m_ext.len = 0;
			}
	   	...
	   	* to get him off.
			...
			else 
			{
				...
				{
	      		while ( ds->q0 ) 
						tp_Abort(ds->q0->sockid);

	      		while (ds->q ) 
						tp_Abort(ds->q->sockid);
	    			} 
				else 
				{
              	if (ds->q0) 
         ...
         if (ds->peer_socket)
			{
				if(ds->peer_socket->peer_socket == ds)
					ds->peer_socket->peer_socket = 0x0;
			
				if(ds->peer_socket->qhead)
					if(ds->peer_socket->qhead->peer_socket == ds)
						ds->peer_socket->qhead->peer_socket = 0x0;
			}
#endif /*GATEWAY*/
	  		
			scheduler.sockets[ds->sockid].ptr = NULL;
  			scheduler.num_of_socket--;
			//printf("tp_Unthread: num_of_socket==%d,ds->sockid=%d!!!\n", scheduler.num_of_socket, ds->sockid);
			if (ds)
				my_free (ds);
...
tp_Flush (tp_Socket * s)
{
  ...
  if (((s->app_sbuff->read_head) &&
      ...
      //(s->app_sbuff->size < s->app_sbuff->max_size)) ||
      ...
      if (s->app_sbuff->size > 0)
	{
	  ...
	  while ((s->send_buff->b_size < s->send_buff->max_size) &&
 	  //while ((s->app_sbuff->size < s->app_sbuff->max_size) &&
		...
		//printf("tp_utility.c 1473,tp_BuildHdr\n");
	  ...
	  (s->state == tp_StateWANTTOLAST))
	{
     ...
     //if (s->app_sbuff->size >= s->app_sbuff->max_size)
	    {
	      free_mbuff (mbuffer);
	      SET_ERR (SCPS_ENOMEM);
	      return (-1);
...
printf("tp_utility.c 1530,tp_BuildHdr\n");
...
void tp_WinAck (tp_Socket * s, tp_Header * th)
{
	...
	if( (!s) || (!th) )
	{
		printf("Gateway-->Fuction tp_WinAck: s=%d, th=%d!\n", (int)s, (int)th );
		fflush(stdout);
		return;
	}
  	...
  	if ((long_temp < (uint32_t) (s->app_rbuff->max_size / 4)) && (long_temp < 1536/*(uint32_t) (s->maxseg)*/))
    	long_temp = 0;
...
/**********Get current year-month-date-hour-minute-second***********/
...
void  time_print(char *timestring)
{
	...
	gettimeofday(&txc, NULL);
	rtc_time_to_tm(txc.tv_sec, &tm);

	sprintf(timestring, "%d%.2d%.2d%.2d%.2d%.2d",tm.tm_year+1900,tm.tm_mon+1,tm.tm_mday, tm.tm_hour,tm.tm_min,tm.tm_sec );
}
...
void generate_bak_log(void)
{
	...
	if(num_of_log_record >= MAX_NUM_OF_LOG_RECORD)
	{
		num_of_log_record = 0;
		...
		system("mv -f /ippep/snmp/tcpinfo.log /ippep/snmp/tcpinfo.log_bak");
...
/**********End current year-month-date-hour-minute-second***********/
...
void check_socket(int location)
{
	...
	for (s = tp_allsocs; s != NIL; s = s->next)
	{
		if (s->prev) 
		{
			if (s != s->prev->next) 
			{
				printf ("%s %d sockets prev location=%d, sockid=%i!\n",__FILE__, __LINE__, location, s->sockid);
				fflush(stdout);
			}
		}
		...
		if (s->next) 
		{
			if (s->next->prev != s) 
			{
				printf ("%s %d sockets next location=%d, sockid=%i!\n",__FILE__, __LINE__, location, s->sockid);
				fflush(stdout);
			}
		}
	}
