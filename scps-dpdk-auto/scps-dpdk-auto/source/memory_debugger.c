...
registerBlock (void *p, size_t b, char *file, int line)
{
   ...
   *    Use real my_malloc and realloc here because we don't want to call ourselves!
   */
  if (currentInfoStruct >= numInfoStructs) {
    numInfoStructs += 1000;
    if (info == NULL) {
      info = (MEMORYALLOCATIONINFO *) my_malloc (numInfoStructs * sizeof (MEMORYALLOCATIONINFO));
    } else {
...
{
  *theInfo = (MEMORYALLOCATIONINFO *) my_malloc (currentInfoStruct * sizeof (MEMORYALLOCATIONINFO));
...
unregisterBlock (void *p, char *file, int line)
{
  ...
  if (i < 0) {
    fprintf (stderr,
	     "Trying to my_free something we didn't allocate from [%s/%d] at %p call(%d)\n",
