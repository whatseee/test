 ...
 */
void tp_Handler (scps_np_rqts * rqts, int len, tp_Header * tp, int *already_freed)
{
	tp_PseudoHeader ph;
	tp_Socket *s;
	int tp_header_len = 0;
	...
	char timestring[LEN_OF_TIME_STR];

	struct timeval mytime;

	int local_cnt = 0;      //mpls
	int ii = 0;		  //mpls
	...
	if (rqts->nl_protocol == NL_PROTOCOL_NP) 
	{
		...
		{
			for (s = tp_allsocs; s; s = s->next)
				if (s->hisport != 0 && tp->dstPort == s->myport && tp->srcPort == s->hisport && htonl (rqts->ipv4_src_addr) == s->his_ipv4_addr)
				{
					break;
				}
	...
	if (rqts->nl_protocol == NL_PROTOCOL_IPV4) 
	{
		...
		//{
#if 0			
			fprintf(fp_cksum, "CRC error: There is something wrong in CRC checksum!\n");
			for (s = tp_allsocs; s; s = s->next)
				if (s->hisport != 0 && tp->dstPort == s->myport && tp->srcPort == s->hisport && htonl (rqts->ipv4_src_addr) == s->his_ipv4_addr)
				{
					fprintf(fp_cksum, "srcIP = 0x%x, dstIP =0x%x, srcport = %d, dstport = %d, seqnum = 0x%x!\n ", rqts->ipv4_src_addr, rqts->ipv4_dst_addr, tp->srcPort, tp->dstPort, htonl (tp->seqnum));
					break;
				}
		...
		if (checksum ((word *) & ph, 14) != 0xffff)
		{
			...
			//fprintf(fp_cksum, "SUM error: There is something wrong in IP checksum!\n");    // YGX
			//fflush(fp_cksum);
	...
	if (rqts->nl_protocol == NL_PROTOCOL_IPV6) 
	{
		...
		{
			for (s = tp_allsocs; s; s = s->next)
				if (s->hisport != 0 && tp->dstPort == s->myport && tp->srcPort == s->hisport && memcmp(&(rqts->ipv6_src_addr.addr), &(s->his_ipv6_addr.addr) , sizeof (struct ipv6_addr)))
				{
					break;
				}
	...
	for (s = tp_allsocs; s; s = s->next) 
	{
		if ( ((rqts->nl_protocol == NL_PROTOCOL_NP)&&(s->nl_protocol_id == NL_PROTOCOL_NP)) || ((rqts->nl_protocol == NL_PROTOCOL_IPV4)&&(s->nl_protocol_id == NL_PROTOCOL_IPV4))) 
		{
			if ( 	(s->hisport != 0) &&	
					(tp->dstPort == s->myport) && 
					...
					(htonl(rqts->ipv4_src_addr) == s->his_ipv4_addr)
#ifdef TAP_INTERFACE
					&&( ((s->TPID==0x8100)&&(rqts->TPID==s->TPID)&&(rqts->TCI==s->TCI)) || ((s->TPID==0)&&(s->TCI==0)&&(rqts->TPID==0)&&(rqts->TCI==0)) )
#endif
				)
			{
#ifdef TAP_INTERFACE
				if( (s->mpls_depth_r>0) && (s->mpls_depth_r<=MAX_MPLS_DEPTH) )
				{
					if(s->mpls_depth_r == rqts->mpls_depth)
					{
						local_cnt = 0;
						ii=0;

						for(ii=0; ii<s->mpls_depth_r; ii++)
						{
							//if(s->mpls_r[ii] == rqts->mpls[ii])
							if((s->mpls_r[ii] && 0xfffff100) == (rqts->mpls[ii] && 0xfffff100))
								local_cnt++;
						}

						if(local_cnt == s->mpls_depth_r)
							break;
					}
				}
				else
		...
		if ((rqts->nl_protocol == NL_PROTOCOL_IPV6) && (s->nl_protocol_id == NL_PROTOCOL_IPV6)) 
		{
			if (s->hisport != 0 &&  tp->dstPort == s->myport &&  tp->srcPort == s->hisport && (!memcmp (&(rqts->ipv6_src_addr.addr),&(s->his_ipv6_addr.addr),sizeof (struct ipv6_addr))))
			{
				break;
	...
	if (s == NIL)
		...
		{
			other_proto_Handler(rqts->interface, &in_data, MAX_MTU/*Be not used in the func*/, NULL/*Be not used in the func*/, rqts, SCPSTP);
			*already_freed = 1;
			return;
		}
...
#ifdef GATEWAY_STRICT
{
		...
		if (total_needed + sys_memory.clust_promised >  sys_memory.fclist.max_size)
		{
			other_proto_Handler(rqts->interface, &in_data, MAX_MTU/*Be not used in the func*/, NULL/*Be not used in the func*/, rqts, SCPSTP);
			*already_freed = 1;
			return;
...
*/
...
#ifdef GATEWAY
		if ( (gw_ifs.c_clust_thresh) && (sys_memory.clust_in_use > gw_ifs.c_clust_thresh)) 
		{
			struct stat sb;
		...
		//printf("clust_in_use=%d, fclist.max_size=%d!!!\n", sys_memory.clust_in_use, sys_memory.fclist.max_size );
		if (/*1*/	
#if 1
				(sys_memory.clust_in_use  + 1000/*The old value is 20*/ >= sys_memory.fclist.max_elements) || 
				(reset_now) || 
				(scheduler.num_of_socket + 32 >= MAX_SCPS_SOCKET) 
#endif
			)
        	{
			other_proto_Handler(rqts->interface, &in_data, MAX_MTU/*Be not used in the func*/, NULL/*Be not used in the func*/, rqts, SCPSTP);
			*already_freed = 1;
			//printf("tp_handler: num_of_socket=%d!!!\n", scheduler.num_of_socket );
			return;
/*
...
/* First Create passive socket */
		...
		if ((s1 = scps_socket (AF_INET, SOCK_STREAM, 0)) == -1)
		{
			...
			time_print(timestring);
			log_msg("%s [IPPEP][TCP] 10 01 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 Building of the TCP connection is failed.(Passive socket creating failed)\n",
				timestring,
				(rqts->ipv4_dst_addr)>>24 & 0x000000ff,
				(rqts->ipv4_dst_addr)>>16 & 0x000000ff,
				(rqts->ipv4_dst_addr)>>8 & 0x000000ff,
				rqts->ipv4_dst_addr & 0x000000ff,
				ntohs(tp->dstPort),
				(rqts->ipv4_src_addr)>>24 & 0x000000ff,
				(rqts->ipv4_src_addr)>>16 & 0x000000ff,
				(rqts->ipv4_src_addr)>>8 & 0x000000ff,
				rqts->ipv4_src_addr & 0x000000ff,
				ntohs(tp->srcPort) );
		...
		if ((rc = scps_bind (s1, (struct sockaddr *) &dst, sizeof (dst))) == -1)
		{
			...
			time_print(timestring);
			log_msg("%s [IPPEP][TCP] 10 01 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 Building of the TCP connection is failed.(Passive socket binding failed)\n",
				timestring,
				(rqts->ipv4_dst_addr)>>24 & 0x000000ff,
				(rqts->ipv4_dst_addr)>>16 & 0x000000ff,
				(rqts->ipv4_dst_addr)>>8 & 0x000000ff,
        ...
        if (rqts->nl_protocol == NL_PROTOCOL_IPV6) 
		{
			...
			if ((rc = scps_bind (s1, (struct sockaddr *) &dst6, sizeof (dst6))) == -1)
			{
		...
		s->mpls_depth_r = rqts->mpls_depth;
		for(local_cnt=0; local_cnt<rqts->mpls_depth; local_cnt++)
		{
			s->mpls_r[local_cnt] = rqts->mpls[local_cnt];
		}
...
//      clear_timer (s->otimers[Rexmit], 1); PDF XXX WHY IS THIS HERE
...
#ifndef IPV6
		{
			...
			while (!port_ok)
			{
				...
				if (tp_allsocs)
				{
					for (temp_socket = tp_allsocs; temp_socket != NIL; temp_socket = temp_socket->next)
					{
					...
					if (!port_ok)
					{
						...
						if (initial_port == tp->srcPort + init_port_number_offset)
						{
							port_ok = 1;
							close_s1 = 1;
						}
			...
			if ((s2 = scps_socket (AF_INET, SOCK_STREAM, 0)) == -1)
			{
				...
				time_print(timestring);
				log_msg("%s [IPPEP][TCP] 10 01 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 Building of the TCP connection is failed.(Active socket creating failed)\n",
					timestring,
					(rqts->ipv4_src_addr)>>24 & 0x000000ff,
					(rqts->ipv4_src_addr)>>16 & 0x000000ff,
					(rqts->ipv4_src_addr)>>8 & 0x000000ff,
			...
			else
			{
				...
				if ((rc = scps_bind (s2, (struct sockaddr *) &src, sizeof(src))) == -1)
				{
					...
					time_print(timestring);
					log_msg("%s [IPPEP][TCP] 10 01 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 Building of the TCP connection is failed.(Active socket binding failed)\n",
						timestring,
						(rqts->ipv4_src_addr)>>24 & 0x000000ff,
						(rqts->ipv4_src_addr)>>16 & 0x000000ff,
						(rqts->ipv4_src_addr)>>8 & 0x000000ff,
				...
				active_s->TPID = rqts->TPID;
				active_s->TCI = rqts->TCI;
				active_s->frame_type = rqts->frame_type;

				active_s->mpls_depth_s = rqts->mpls_depth;
				for(local_cnt=0; local_cnt<rqts->mpls_depth; local_cnt++)
				{
					active_s->mpls_s[local_cnt] = rqts->mpls[local_cnt];
				}
				...
				mytime.tv_sec = 60;
				mytime.tv_usec = 0;
				set_timer(&mytime, active_s->otimers[Keep], 1);
			}
		}
#else /* IPV6 */
        {
			if (rqts->nl_protocol == NL_PROTOCOL_IPV4) 
			{
				...
				else
				{
					...
					for(local_cnt=0; local_cnt<rqts->mpls_depth; local_cnt++)
					{
						active_s->mpls[local_cnt] = rqts->mpls[local_cnt];
					}
			...
			if (rqts->nl_protocol == NL_PROTOCOL_IPV6) 
			{
				...
				else
				{
					...
					if ((rc = scps_bind (s2, (struct sockaddr *) &src6, sizeof(src6))) == -1)
					{
						syslog (LOG_ERR, "Gateway: not allowing any more connection sb2\n");
						close_s1 = 1;
						scps_close (active_s->sockid);
...
/*
		...
		if(tp_header_len > 20)
		{
			printf("Gateway-->Function tp_handler: memcpy more than 20(%d) bytes to s->in_th!\n", tp_header_len);
			fflush(stdout);
		}
...
int tp_CommonHandler (tp_Socket * s, 
					  scps_np_rqts * rqts, 
					  tp_Header * tp,
					  byte * data, 
					  int len)
{
   ...
   char timestring[LEN_OF_TIME_STR];
	...
	unsigned short rbuf_size = 0;

	int local_cnt = 0;               //mpls

	mytime.tv_sec = mytime.tv_usec = 0;

	if ((len != 0) && (sys_memory.clust_in_use + 100 >= sys_memory.fclist.max_elements)) 
	{
		return (-1);
	}


	//modified on 20131124 as follows:
#if 0
	if ((len != 0) && (sys_memory.clust_in_use + 100 >= sys_memory.fclist.max_elements)) 
	{
#ifdef GATEWAY
	...
	if (flags & tp_FlagRST)
		...
		{
			time_print(timestring);
			log_msg("%s [IPPEP][TCP] 10 02 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 This tcp connection is resetted.\n",
				timestring,
				(ntohl(s->his_ipv4_addr)>>24) & 0x000000ff,
				(ntohl(s->his_ipv4_addr)>>16) & 0x000000ff,
				(ntohl(s->his_ipv4_addr)>>8) & 0x000000ff,
				ntohl(s->his_ipv4_addr) & 0x000000ff,
				ntohs(s->hisport),
				(ntohl(s->my_ipv4_addr)>>24) & 0x000000ff,
				(ntohl(s->my_ipv4_addr)>>16) & 0x000000ff,
				(ntohl(s->my_ipv4_addr)>>8) & 0x000000ff,
				ntohl(s->my_ipv4_addr) & 0x000000ff,
				ntohs(s->myport) );
		...
		{
			time_print(timestring);
			log_msg("%s [IPPEP][TCP] 10 01 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 5 Building of the TCP connection is failed.(Receive RST during 3-times-handshake).\n",
				timestring,
				(ntohl(s->his_ipv4_addr)>>24) & 0x000000ff,
				(ntohl(s->his_ipv4_addr)>>16) & 0x000000ff,
				(ntohl(s->his_ipv4_addr)>>8) & 0x000000ff,
		...
		if ((s->thread->status == Blocked) && ((s->read) || (s->write)))
		{
...
#else /* GATEWAY_SELECT */
		if ((s->thread->status == Blocked) && ((scheduler.sockets[s->sockid].read) || (scheduler.sockets[s->sockid].write)))
		{
			s->thread->status = Ready;
			scheduler.num_runable++;
	...
	if (option_len && (s->state != tp_StateLISTEN))
		tp_dooptions (s, option_len, tp, &ts_present, &ts_val, &ts_ecr, &rbuf_size);
	...
	if (s->capabilities & CAP_TIMESTAMP)
		...
		if (ts_present && SEQ_GEQ (ts_val, s->ts_recent) && SEQ_LEQ (flippedseq, s->lastack))
		{
...
NewConnect:
...
#if 1
				s->np_rqts.sock = s;
				new_socket->np_rqts.sock = new_socket;
				new_socket->peer_socket->np_rqts.sock = new_socket->peer_socket;
				new_socket->peer_socket->np_rqts.sock = new_socket->peer_socket;

				new_socket->remote_flag = 2;
				new_socket->peer_socket->remote_flag = 2;
				s->remote_flag = 2;
				s->peer_socket->remote_flag = 2;
#endif
				...
				new_socket->sh_off = new_socket->th_off - new_socket->sp_size - (new_socket->sp_size % sizeof (uint32_t));
...
#undef OLD_CODE 
				...
				tp_dooptions (new_socket, option_len, tp, &ts_present, &ts_val, &ts_ecr, &rbuf_size);
...
#ifdef GATEWAY_ROUTER
    			new_socket->rt_route = route_rt_lookup_s (new_socket);
				s->rt_route = new_socket->rt_route;
#endif /* GATEWAY_ROUTER */
				...
				if (new_socket->capabilities & CAP_JUMBO)
				{
					...
					/****************************** Use CETC54 Sending Speed Notification ******************************/
					if(new_socket->divert_port == gw_ifs.bif_divport)
					{
						if(new_socket->rt_route != NULL)
						{
							buffer_size = new_socket->rt_route->rate / 8;
							buffer_size = max(buffer_size, 65537);
							scps_setsockopt( new_socket->sockid, SCPS_SOCKET, SCPS_SO_SNDBUF, &buffer_size, sizeof(int) );

							buffer_size = max(new_socket->rt_route->rate/8, 1000*(unsigned int)rbuf_size);
							buffer_size = max(buffer_size, 65537);
							scps_setsockopt( new_socket->sockid, SCPS_SOCKET, SCPS_SO_RCVBUF, &buffer_size, sizeof(int) );	

							//new_socket->request_r_scale = 9;
							//new_socket->rcv_scale = 9;
							
							//printf("tp_StateLISTEN: SNDBUF=%d, RCVBUF=%d, rbuf_size=%d!!!\n", new_socket->rt_route->rate/8, max(new_socket->rt_route->rate/8, 1000*(unsigned int)rbuf_size), rbuf_size);
						}
					}
					/****************************** Use CETC54 Sending Speed Notification ******************************/
				} 
				...
				/* The listening socket should be closed after the connection has been fully established */
	...
	if (flags & tp_FlagSYN)
	  {
		...
		clear_timer (s->otimers[Rexmit], 1);
                ...
                mytime.tv_sec = 1;  /* Initially I want to send an ACK fairly soon
 				       ...
 				       lost -- PDF */
		//printf("StateSYNSENT: s->divert_port=%d, gw_ifs, aif_divport=%d, gw_ifs.bif_divport=%d\n", s->divert_port, gw_ifs.aif_divport, gw_ifs.bif_divport);
		if (s->divert_port == gw_ifs.aif_divport)		
            		set_timer (&mytime, s->otimers[KA_L], 1);
		if (s->divert_port == gw_ifs.bif_divport)		
            		set_timer (&mytime, s->otimers[KA_W], 1);
...
////////////////////////////////////////////////////////////////////////////////
		...
		if(s->peer_socket)
		{
			s->peer_socket->mpls_depth_s = rqts->mpls_depth;
			for(local_cnt=0; local_cnt<rqts->mpls_depth; local_cnt++)
			{
				s->peer_socket->mpls_s[local_cnt] = rqts->mpls[local_cnt];
			}
		}
#endif
///////////////////////////////////////////////////////////////////////////
				if(s->peer_socket->state == tp_StateESTAB)
				{
					time_print(timestring);
					log_msg("%s [IPPEP][TCP] 10 03 %d.%d.%d.%d %d %d.%d.%d.%d %d 6 6 Building of the TCP connection is successful.\n",
						timestring,
						(ntohl(s->my_ipv4_addr)>>24) & 0x000000ff,
						(ntohl(s->my_ipv4_addr)>>16) & 0x000000ff,
						(ntohl(s->my_ipv4_addr)>>8) & 0x000000ff,
					...
					//printf("The num of sockets is %d!!!\n", scheduler.num_of_socket);
		...
		if (s->capabilities & CAP_JUMBO)
		  {
			...
			if(s->divert_port == gw_ifs.bif_divport)
			{
				if(s->rt_route != NULL)
				{
					buffer_size = s->rt_route->rate / 8;
					buffer_size = max(buffer_size, 65537);
					scps_setsockopt( s->sockid, SCPS_SOCKET, SCPS_SO_SNDBUF, &buffer_size, sizeof(int) );

					buffer_size = max(s->rt_route->rate/8, 1000*(unsigned int)rbuf_size);
					buffer_size = max(buffer_size, 65537);
					scps_setsockopt( s->sockid, SCPS_SOCKET, SCPS_SO_RCVBUF, &buffer_size, sizeof(int) );	

					//printf("tp_StateSYNSENT: SNDBUF=%d, RCVBUF=%d, rbuf_size=%d!!!\n", s->rt_route->rate/8, max(s->rt_route->rate/8, 1000*(unsigned int)rbuf_size), rbuf_size);
				}
			}
			/****************************** Use CETC54 Sending Speed Notification ******************************/
...
/****************************************** for vipersat *******************************************/
		...
		s->peer_socket->frame_type = rqts->frame_type; // added by lcq on 20140905
		s->peer_socket->mpls_depth_s = rqts->mpls_depth;
#endif /* TAP_INTERFACE */
/****************************************** for vipersat *******************************************/
...
/****************************************** for vipersat *******************************************/
				printf("tp_handle.c 2056,tp_BuildHdr\n");
		...
		if ((s->thread->status == Blocked) &&
		    (((s->write) &&
		      ((s->send_buff->max_size - (s->seqnum - s->snduna)) >=
				//((s->app_sbuff->max_size - (s->seqnum - s->snduna)) >=
		       s->write))))
		  {
		    s->thread->status = Ready;
		    scheduler.num_runable++;
...
printf("tp_handle.c 2135,tp_BuildHdr\n");
...
printf("tp_handle.c 2178,tp_BuildHdr\n");
			if ((mbuffer = tp_BuildHdr (s, NULL, 0)))
			  {
			    /* 
...
#ifdef TAP_INTERFACE
		if (s->rt_route->SMTU) { 
			s->maxdata = min (s->maxdata, s->rt_route->SMTU - tp_hdr_size () - s->np_size - s->sp_size - TP_HDR_LEN - s->mpls_depth_s*SIZE_OF_MPLS_TAG);
		}
#else
		if (s->rt_route->SMTU) {
...
//           s->maxdata -= s->rt_route->MSS_FF;
	    ...
	    //s->snd_cwnd = s->snd_prevcwnd = min (4 * s->maxdata, max (2 * s->maxdata, 4380));
		if(s->divert_port == gw_ifs.bif_divport)
			s->snd_cwnd = s->snd_prevcwnd = s->rt_route->rate / 32;
		else
			s->snd_cwnd = s->snd_prevcwnd = min (4 * s->maxdata, max (2 * s->maxdata, 4380));
#else /* INIT_CWND_INCR */
	   ...
	   s->snd_cwnd = s->snd_prevcwnd = SET_INITIAL_CWND;
	    ...
	    s->state = tp_StateESTAB;
            	...
            	mytime.tv_sec = 1;  /* Initially I want to send an ACK fairly soon
 				   incase the ACK that opens up the window gets
 				   lost -- PDF */
		//printf("StateSYNREC: s->divert_port=%d, gw_ifs, aif_divport=%d, gw_ifs.bif_divport=%d\n", s->divert_port, gw_ifs.aif_divport, gw_ifs.bif_divport);
		if (s->divert_port == gw_ifs.aif_divport)		
            		set_timer (&mytime, s->otimers[KA_L], 1);
		if (s->divert_port == gw_ifs.bif_divport)		
            		set_timer (&mytime, s->otimers[KA_W], 1);
...
if(s->app_rbuff->write_off < 0)
{
	printf("Write_off Error: File %s, Line %d, write_off = %d!\n", __FILE__, __LINE__, s->app_rbuff->write_off);
}
...
if(s->app_rbuff->write_off < 0)
{
	...
	s->lastuwe = s->acknum + s->rcvwin;
printf("tp_handle.c 2843,tp_BuildHdr\n");
	if ((mbuffer = tp_BuildHdr (s, NULL, 0)))
	  {
	    tp_NewSend (s, mbuffer, false);
	    free_mbuff (mbuffer);
...
void gateway_move_data (from, to)
tp_Socket *from;
tp_Socket *to;
{
...
#define BUFFER_SIZ 8192
	...
	if( (!from) || (!to) )
	{
		printf("Gateway-->Function gateway_move_data: from=%d, to=%d!\n", (int)from, (int)to );
		fflush(stdout);
		return;
	}
	...
	//if (sys_memory.clust_in_use + 10 >= sys_memory.fclist.max_size)		//modified on 20131124
	if (sys_memory.clust_in_use + 100 >= sys_memory.fclist.max_elements)
    	{
		from->gateway_flags |= GATEWAY_MORE_TO_WRITE;
		return;
    	}
...
#define ORIG_CODE
		...
		if(to->remote_flag == 0)
		{
			if(to->is_bbr_sk == 10000 )
			{
				if(bbr->cwnd - bbr->cwndUsed > 0)
				{
					bbr->cwndFree = bbr->cwnd - bbr->cwndUsed;
					if(bbr->cwndFree < 0)
					{
						bbr->cwndFree = 0;
					}
					can_be_read = min(from->app_rbuff->size,bbr->cwndFree);
					//printf("fe=%u  ",from->app_rbuff->size);
				}
				else
				{
					can_be_read = 0;
				}
				//printf("%d %d %d  %d\n",bbr->cwnd, bbr->cwndUsed,bbr->cwndFree, can_be_read);
			}
			
		}
		can_be_written = to->app_sbuff->max_size - (to->app_sbuff->size + to->send_buff->data_size);
		/*i++;
		if (i%10 == 0)
		{
			printf("app_sbuff->size = %d, send_buff->data_size = %d, b_size = %d\n", to->app_sbuff->size, to->send_buff->data_size, to->send_buff->b_size);
		}	*/	
...
#ifdef GATEWAY
{
		...
		while (((SMCLBYTES - to->app_sbuff->write_off) + to->app_sbuff->bytes_beyond) < (0 + to_be_moved + temp))
		{
...
#ifdef ORIG_CODE
			if (!((to->app_sbuff->num_elements < to->app_sbuff->max_elements) && (grow_chain (to->app_sbuff, 1))))
			{
			...
			if (to_be_moved)
			{
				...
				if (bytes_read != to_be_moved)
				{
					printf ("READ FAILED err(%d) to_be_moved(%d) bytes_read(%d) tmp(%d) from->state(%d)\n",
						GET_ERR (),to_be_moved,bytes_read,tmp,from->state);
					...
					while(bytes_read > 0)
					{
						tmp = scps_write (to->sockid, (temp_buff+i), bytes_read);
#if 1
						if(to->remote_flag == 0)
						{
							if(to->is_bbr_sk == 10000 )
							{
								bbr->cwndUsed += tmp;
								if(bbr->cwndUsed > bbr->cwnd)
									printf("errrrrrr\n");
								//log4c(__FILE__,__FUNCTION__,"in move bbr->cwndUsed %u\n",bbr->cwndUsed);
								//log4c(__FILE__,__FUNCTION__,"  tmp=  %u\n",tmp);
								//log4c(__FILE__,__FUNCTION__,"  bytes_read=  %u\n",bytes_read);
							}
						}
#endif
						...
						i += tmp;
						if(bytes_read > 0)
							printf ("WRITE FAILED 1 err(%d) bytes_read(%d) tmp(%d)\n", GET_ERR (), bytes_read, tmp);
					}
					i = 0;
				}
				else
...
void gateway_set_options (sockid, divert_port, other)
int sockid;
int divert_port;
int other;
{
  	...
  	printf("%s %s gateway_set_options sockid(%d) divert_port(%d) other(%d)\n", stringNow(), printPorts(s), sockid, divert_port, other);
#endif /* DEBUG_GATEWAY */

  	switch (other)
    	{
    	case 0:			/* This means the divert port corresponds to the interface this was received on*/
							/* Note: This means the divert port (port which receives this packet) and the socket are on the same interface*/
      	...
      	if(divert_port == gw_ifs.aif_divport)
			{
	  			...
	  			if (gw_ifs.aif_rbuf) 
				{
	  			...
	  			scps_setsockopt (sockid, SCPS_SOCKET, SCPS_SO_RCVBUF, &buffer_size, sizeof(buffer_size));
	  			...
	  			switch (gw_ifs.aif_cc)
	    			...
	    			case VEGAS_CONGESTION_CONTROL:
              		...
              		if (gw_ifs.aif_vegas_alpha)  
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_ALPHA, &gw_ifs.aif_vegas_alpha, sizeof (gw_ifs.aif_vegas_alpha));

              		if (gw_ifs.aif_vegas_beta)  
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_BETA, &gw_ifs.aif_vegas_beta, sizeof (gw_ifs.aif_vegas_beta));

              		if (gw_ifs.aif_vegas_gamma)  
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_GAMMA, &gw_ifs.aif_vegas_gamma, sizeof (gw_ifs.aif_vegas_gamma));
              			
						scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_SS, &gw_ifs.aif_vegas_ss, sizeof(gw_ifs.aif_vegas_ss));
	      			break;

	    			case FLOW_CONTROL_CONGESTION_CONTROL:
	      			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_FLOW_CONTROL_CONGEST, &one, sizeof(one));
	      			break;
	    			}
            ...
            s->sp_rqts.secure_gateway_rqts=gw_ifs.aif_scps_security;
           	...
           	if (gw_ifs.aif_2msltimeout != 0) 
				{
           		scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_2MSLTIMEOUT, &gw_ifs.aif_2msltimeout, sizeof (gw_ifs.aif_2msltimeout));
	   			}

           	scps_setsockopt (sockid, SCPS_SOCKET, SCPS_SO_NLDEFAULT, &gw_ifs.aif_nl, sizeof(gw_ifs.aif_nl));
	  			...
	  			switch (gw_ifs.bif_cc)
	    			...
	    			case VEGAS_CONGESTION_CONTROL:
              		...
              		if(gw_ifs.bif_vegas_alpha)  
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_ALPHA, &gw_ifs.bif_vegas_alpha, sizeof (gw_ifs.bif_vegas_alpha));
              		if(gw_ifs.bif_vegas_beta)
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_BETA, &gw_ifs.bif_vegas_beta, sizeof (gw_ifs.bif_vegas_beta));
              		if(gw_ifs.bif_vegas_gamma)  
	          			scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_GAMMA, &gw_ifs.bif_vegas_gamma, sizeof (gw_ifs.bif_vegas_gamma));
              		scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_VEGAS_SS, &gw_ifs.bif_vegas_ss, sizeof (gw_ifs.bif_vegas_ss));
	      			break;
            ...
            s->sp_rqts.secure_gateway_rqts=gw_ifs.bif_scps_security;
           	...
           	if(gw_ifs.bif_2msltimeout != 0) 
				{
           		scps_setsockopt (sockid, PROTO_SCPSTP, SCPSTP_2MSLTIMEOUT, &gw_ifs.bif_2msltimeout, sizeof (gw_ifs.bif_2msltimeout));
	   			}

           	scps_setsockopt (sockid, SCPS_SOCKET, SCPS_SO_NLDEFAULT, &gw_ifs.bif_nl, sizeof(gw_ifs.bif_nl));
...
/* Assign the routed properly now */
    	...
    	case 1:			/* This means the divert port corresponds to the other interface this was received on */
							/* Note: This means the divert port (port which receives this packet) and the socket are on the different interface*/
