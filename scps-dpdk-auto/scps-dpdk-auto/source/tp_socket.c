...
clone_socket (tp_Socket * socket)
{
  ...
  tp_Socket *new_socket = (tp_Socket *) my_malloc (sizeof (tp_Socket));
  ...
  for (temp = 0; temp < MAX_SCPS_SOCKET; temp++)
      ...
      if (scheduler.sockets[temp].ptr == NULL)
	{
	  ...
	  //printf("clone_socket: num_of_socket==%d !!!\n", scheduler.num_of_socket);
	  break;
	}
    }
  ...
  new_socket->brtt = 0;
	new_socket->is_bbr_sk = 0;
	new_socket->pacing_rate = 0;
  bbr_init(new_socket);
...
SockAlloc_Failure:
  ...
  if (new_socket->receive_buff)
    my_free (new_socket->receive_buff);
  if (new_socket->Out_Seq)
    my_free (new_socket->Out_Seq);
  if (new_socket->send_buff)
    my_free (new_socket->send_buff);
  ...
  if (new_socket->app_rbuff) {
    ...
    my_free (new_socket->app_rbuff);
  }

  my_free (new_socket);
  scheduler.sockets[new_socket->sockid].ptr = 0x0;
  scheduler.num_of_socket--;
  //printf("clone_socket failed: num_of_socket==%d !!!\n", scheduler.num_of_socket);
  SET_ERR (SCPS_ENOMEM);
  return (NULL);
}
...
int scps_socket (int family, int type, int flags)
{
	...
	switch (type)
    {
		...
		case SOCK_SEQPACKET:
		case SOCK_ROUTE:
			socket = (void *) my_malloc (sizeof (tp_Socket));
		...
		case SOCK_DGRAM:
			socket = (void *) my_malloc (sizeof (udp_Socket));

			if(!socket)
			{
				printf("Gateway-->Function scps_socket: my_malloc return NULL!\n");
				fflush(stdout);
				return -1;
			}
...
{
  	struct protoent *entity = (struct protoent *) my_malloc (sizeof (struct protoent));
  	SET_ERR (0);

	if(!entity)
	{
		printf("Gateway-->Function scps_getprotobyname: my_malloc return NULL!\n");
		fflush(stdout);
		return NULL;
	}
	...
	{
		printf("Gateway-->Function scps_getprotobyname: add my_free!\n");
		my_free(entity);
...
#ifdef GATEWAY_ROUTER   //added by CTI
    	s->rt_route = route_rt_lookup_s (s);
#endif /* GATEWAY_ROUTER */
//printf("tp_socket.c 1575,tp_BuildHdr\n");
  if ((s->send_buff->b_size < s->send_buff->max_size) && (mbuffer = tp_BuildHdr (s, NULL, 0)))
  //if ((s->send_buff->data_size < s->send_buff->max_size) && (mbuffer = tp_BuildHdr (s, NULL, 0)))
    {
...
int tp_Abort (int sockid)
{
   struct mbuff *mbuffer;
	...
	if ( /*(s->state != tp_StateLISTEN) && */(s->state != tp_StateCLOSED) && (s->state != tp_StateTIMEWT) )
    {
		...
		* mbuffer
		*/
		//printf("tp_socket.c 1791,tp_BuildHdr\n");
      ...
      /* Check to see if s is on socs_to_reset */
          ...
          if (found) {
              ...
              new  = (reset_Socket*) my_malloc (sizeof (reset_Socket));
              new ->s = s;
              new ->next = NULL;
              if (socs_to_reset) {
...
tp_Write (int sockid, void *dp, int len, int push)
{
  ...
  if (((s->app_sbuff->size + s->send_buff->data_size + len) > s->app_sbuff->max_size) 
  //if (((s->app_sbuff->size + len) > s->app_sbuff->max_size) 
       || (!((len = cb_cpdatin (s->app_sbuff, dp, len, 0, s->maxdata)) > 0)))
    {
      if (s->sockFlags & SOCK_BL)
	{
	  ...
	  while ((len > (s->app_sbuff->max_size - (s->app_sbuff->size + s->send_buff->data_size))) ||
     //while ((len > (s->app_sbuff->max_size - s->app_sbuff->size)) ||
		 (!((len = cb_cpdatin (s->app_sbuff, dp, blen, 0, s->maxdata)) > 0)))
	    {
	      sched ();
	      /* We might have lost the connection while sleeping... */
	      if (s->state == tp_StateCLOSED)
  ...
  while (count > 0)
    {
      ...
      if ((s->send_buff->b_size >= s->send_buff->max_size) || (!(mbuffer = alloc_mbuff (MT_HEADER))))
		//if ((s->app_sbuff->size >= s->app_sbuff->max_size) || (!(mbuffer = alloc_mbuff (MT_HEADER))))
	{
	  if (s->sockFlags & SOCK_BL)
	    {
	      /* 
...
//printf("tp_socket.c 2174,tp_BuildHdr\n");
...
#endif /* OPT_RECORD_BOUNDARY */
	/*if (((4294967295 - 2000) <= s->send_buff->send->m_seq) && (s->send_buff->send->m_seq < 4294967295))
		if (s->send_buff->send == s->send_buff->start)
			tp_NewSend (s, NULL, false);
	else*/
	{
	  if (s->send_buff->send)
...
tp_Read (int sockid, void *data, int size)
{
  ...
  switch (sock->state)
	...
	if ((sock->capabilities & CAP_BETS) &&
	  ...
	  BETS_NOTIFY:
	     ...
	     * so we can my_free the space in the buffers we were reserving
	  ...
	  if (sock->app_rbuff->RB->next)
	    {
	      ...
	      my_free (sock->app_rbuff->RB->prev);
	    }
	  else
	    {
	      my_free (sock->app_rbuff->RB);
  ...
  if (				/* (size >= (sock->maxdata << 1)) || */
      ((old_win < /*sock->my_mss_offer*/1536) && (sock->rcvwin >= /*sock->my_mss_offer*/1536)))
    {
		sock->flags = tp_FlagACK;
		//printf("tp_socket.c 2713,tp_BuildHdr\n");
		if ((mbuffer = tp_BuildHdr (sock, NULL, 0)))
	    	{
	      if (tp_NewSend (sock, mbuffer, false) > sock->np_size)
			{
		  		sock->unacked_segs = 0;
		  		sock->lastuwe = sock->acknum + sock->rcvwin;
		  		sock->lastack = sock->acknum;
		  		sock->ack_delay = 0;
			}
			else
			{
