...
SCPS_logRate(route *r)
{
  ...
  for (s = tp_allsocs; s; s = s->next) {
    ...
    if ( (int)(theInfo->last_rate_value)==s->rt_route->current_credit ) return;
    if ( theInfo->initialized==XPLOT_INIT_VALUE ) {
      /* Log rate to xplot file here. */
      if ( (int)(theInfo->last_rate_value)!=s->rt_route->current_credit ) {
  ...
  while (m)
    {
      ...
      printf ("Walked %s more than %d:: mbuff at %p forward(%p) reverse(%p) parent(%p)",
	      ...
	      m, m->pep_m_next, m->m_prev, m->parent);
...
mbuff_pointer_walk (struct mbuff *m, int size, char *file, int line)
{
  ...
  while ( temp->pep_m_next )
    {
      BAD_POINTER (temp->pep_m_next);
		  ...
		  tp_Socket * s)
{
  ...
  if (status != CLUSTER_UNKNOWN)
	...
	{
	  printf
	    ("verify_mbcluster: de_queued is NOT set in a cluster we think IS on the my_free list.\n");
	}
      if (m->de_queued && (status == CLUSTER_QUEUED))
	{
	  printf
	    ("verify_mbcluster: de_queued IS set in a cluster we think IS NOT on the my_free list.\n");
	}
    }
...
verify_mbuff (struct mbuff *m, char *file, int line)
{
  ...
  BAD_POINTER (m->pep_m_next);
  if ((m->m_prev) && (m->m_prev->pep_m_next != m))
    {
      printf ("verify_mbuff(%d): Broken links in chain!\n", checking);
      printf
	("               (seq# %lu)(%p) previous neighbor points to %p\n",
	 m->m_seq, m, m->m_prev->pep_m_next);
      error++;
    }
  if ((m->pep_m_next != NULL) && (m->pep_m_next->m_prev != m))
    {
      printf ("verify_mbuff(%d): Broken links in chain!\n", checking);
      printf
	("               (seq# %lu)(%p) next neighbor points back to %p\n",
	 m->m_seq, m, m->pep_m_next->m_prev);
      error++;
    }
 ...
 */
	  ...
	  if (hole->hole_end_seq !=
		  ...
		  || (SEQ_LT (hole->hole_end_seq, hole->hole_end->m_seq)))
		{
		  ...
		  if (hole->hole_end && hole->hole_end->pep_m_next)
		    {
		      logEventv(s, trim,
			       ", hole_end->next's last seq (%lu)\n",
			       hole->hole_end->pep_m_next->m_seq + hole->hole_end->pep_m_next->m_ext.len);
		    }
		  logEventv(s, trim, "\n     hole_idx = %d\n", hole_idx);
#endif /* VERBOSE */
		  retval |= TEST_3_FAIL;
