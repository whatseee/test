...
extern volatile uint8_t quit_signal;
...
create_interface (spanner_ip_addr local_addr, spanner_ip_addr span_addr)
{
  ...
  if (!(local_addr))
    get_local_internet_addr ((char *) &local_addr);
  	...
  	interface = (struct _interface *) my_malloc (sizeof (struct _interface));
	if(!interface)
	{
		printf("Gateway-->Function create_interface: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
	}
...
struct _interface *create_interface_new (void)
{
  	struct _interface *interface, *index;
	...
	if(!interface)
	{
		printf("Gateway-->Function create_interface_new: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
	}
  	memset (interface, 0, sizeof (struct _interface));
  	initialize_interface (interface, 0);
  	index = interface;
  	sock_interface = interface;
...
int ll_tun_send  (struct _interface *interface, 
                  ...
                  route *a_route)
{
    ...
    if (interface == sock_interface) 
	{
       ...
       remote_addr.sin_port = htons (special_port_number);
        ...
        if (special_ip_addr) 
		{
           memcpy ((void *) &remote_addr.sin_addr, (void *) &special_ip_addr, sizeof (special_ip_addr));
        }
...
void create_divert_interface (local_addr, port_number1)
spanner_ip_addr local_addr;
int32_t port_number1;
{
printf("############create_divert_interface###########\n");
    int optlen, rcvbuff, sndbuff;
	struct _interface *interface, *index;
#ifdef __ISBSDISH__
	int hincl = 0;
	...
	if(!interface)
	{
		printf("Gateway-->Function create_divert_interface: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
	}
	memset (interface, 0, sizeof (struct _interface));
    ...
    if (setsockopt (s_command, SOL_SOCKET, SO_REUSEADDR, (char *) &one, sizeof (int)) < 0) 
	{
	...
	if (bind (s_command, (struct sockaddr *) &sin_command,sizeof (sin_command)) < 0) 
	{
        perror ("Binding SCPS RI console socket");
        exit (0);
    	}
	...
	interface->tap_a_fd = 0;
	printf("interface->tap_a_fd: %d\n", interface->tap_a_fd);
	...
	interface->tap_b_fd = 1;
	printf("interface->tap_b_fd: %d\n", interface->tap_b_fd);
	...
	if ((interface->div_socket = socket (AF_INET, SOCK_RAW, IPPROTO_DIVERT)) < 0)
    	{
        syslog (LOG_ERR, "setup_lower:  can't create Divert socket 1\n Are you sure that you have permissions to run over RAW_IP sockets?\n");
		return;
    	}
	interface->div_port = port_number1;

	if ((interface->div_a_socket = socket (AF_INET, SOCK_RAW, IPPROTO_DIVERT)) < 0)
    	{
		syslog (LOG_ERR, "setup_lower:  can't create Divert socket a\n Are you sure that you have permissions to run over RAW_IP sockets?\n");
		return;
   	}
	interface->div_a_port = port_number2;

	if ((interface->div_b_socket = socket (AF_INET, SOCK_RAW, IPPROTO_DIVERT)) < 0)
    	{
        syslog (LOG_ERR, "setup_lower:  can't create Divert socket b\n Are you sure that you have permissions to run over RAW_IP sockets?\n");
		return;
    	}
	interface->div_b_port = port_number3;
	...
	if (bind (interface->div_socket, (struct sockaddr *) &local_sock_addr, sizeof (local_sock_addr)) < 0)
    	{
      err_dump ("setup_lower:  couldn't bind to local address");
    	}
	interface->div_port = port_number1;
	...
	if (bind (interface->div_a_socket, (struct sockaddr *) &local_sock_addr, sizeof (local_sock_addr)) < 0)
    	{
		err_dump ("setup_lower:  couldn't bind to local address");
    	}
	...
	if (bind (interface->div_b_socket, (struct sockaddr *) &local_sock_addr, sizeof (local_sock_addr)) < 0)
    	{
		err_dump ("setup_lower:  couldn't bind to local address");
    	}
	interface->div_b_port = port_number3;
	...
	rcvbuff = LL_BUFFER;
	optlen = sizeof (rcvbuff);
	if (setsockopt (interface->div_socket, SOL_SOCKET, SO_RCVBUF, (char *) &rcvbuff, sizeof (rcvbuff)) < 0)
    fprintf (stderr, "Error a setting receive buffer size to %d  :%d\n", rcvbuff, errno);

	sndbuff = LL_BUFFER;
	optlen = sizeof (sndbuff);
	if (setsockopt (interface->div_socket, SOL_SOCKET, SO_SNDBUF, (char *) &sndbuff, sizeof (sndbuff)) < 0)
    fprintf (stderr, "Error a setting send buffer size to %d\n", sndbuff);

	rcvbuff = LL_BUFFER;
	optlen = sizeof (rcvbuff);
	if (setsockopt (interface->div_a_socket, SOL_SOCKET, SO_RCVBUF, (char *) &rcvbuff, sizeof (rcvbuff)) < 0)
    fprintf (stderr, "Error b setting receive buffer size to %d  :%d\n", rcvbuff, errno);

	sndbuff = LL_BUFFER;
	optlen = sizeof (sndbuff);
	if (setsockopt (interface->div_a_socket, SOL_SOCKET, SO_SNDBUF, (char *) &sndbuff, sizeof (sndbuff)) < 0)
    fprintf (stderr, "Error b setting send buffer size to %d\n", sndbuff);

	rcvbuff = LL_BUFFER;
	optlen = sizeof (rcvbuff);
	if (setsockopt (interface->div_b_socket, SOL_SOCKET, SO_RCVBUF,(char *) &rcvbuff, sizeof (rcvbuff)) < 0)
    fprintf (stderr, "Error c setting receive buffer size to %d  :%d\n", rcvbuff, errno);

	sndbuff = LL_BUFFER;
	optlen = sizeof (sndbuff);
	if (setsockopt (interface->div_b_socket, SOL_SOCKET, SO_SNDBUF, (char *) &sndbuff, sizeof (sndbuff)) < 0)
	...
	printf("############end of divert interface###########\n");
	return;
}
#endif /* ENCAP_DIVERT */
...
#ifdef TUN_INTERFACE
	cc = ll_tun_send  (interface, remote_internet_addr, protocol, data_len, my_msg, a_route);
	return (cc);
#endif /* TUN_INTERFACE */

#ifdef TAP_INTERFACE
	cc = ll_tap_send  (interface, remote_internet_addr, protocol, data_len, my_msg, a_route, rqts);
	return (cc);
#endif /* TAP_INTERFACE */
 ...
 */
int ll_nbreceive (struct _interface *interface,
...
void free_ll_queue_element (struct _interface *interface, struct _ll_queue_element *buffer)
{
	pthread_mutex_lock(&mutex);
	buffer->next = NULL;
	...
	if (interface->available.tail)
	{
		...
		interface->available.head = interface->available.tail =	buffer;
...
void initialize_interface (struct _interface *interface, uint32_t local_addr)
{
	...
	interface->incoming.size = interface->outgoing.size = interface->available.size = 0;
	...
	for (num_elements = 0; num_elements < MAX_LL_QUEUE_ELEMENTS; num_elements++)
	{
        if ((new_element = (struct _ll_queue_element *) my_malloc (sizeof (struct _ll_queue_element))))
		{
...
void service_interface (struct _interface *interface)
{
	...
	struct rte_mbuf *m, s_m;
	unsigned lcore_id;
	unsigned j, portid, nb_rx_pkts;
...
#define MAX_TO_HANDLE 100
	...
	if ((!(interface->is_free)) || (!(interface->available.head)))
		return;
	//printf("service_interface 1111111111111\n");
...
/***************************************dpdk*****************************************/
	...
	//printf("service_interface: entering main loop on lcore %u, portnum:%d\n", lcore_id, nb_ports);
	/*
	 * Read packet from ports
	 */
  //while (1)
  {
      //usleep(10);
	...
	for (portid = 0; portid < nb_ports; portid++) 
	{
		...
		{
			/* receive packets */
			nb_rx_pkts = rte_eth_rx_burst((uint8_t) portid, 0,
							pkts_burst, MAX_PKT_BURST);
			if (nb_rx_pkts == 0) 
			{
				//printf("ll_support.c 1903: port %d Received zero packets\n", portid);
				continue;
			}
			...
			for (j = 0; j < nb_rx_pkts; j++) 
			{
				m = pkts_burst[j];
				to_read = m->port;
				//printf("ll_support.c 1861: port %d recv %d pkts\n", m->port,nb_rx_pkts);

				l2pkt = rte_pktmbuf_mtod(m, unsigned char *);
#if 0
				val = rte_eth_tx_burst(to_read^1, 0, &m, 1);
				if (unlikely(val < 1)) 
				{	
				...
				if (interface->available.head)
				{
					interface->available.head->size = m->data_len;

					memcpy(interface->available.head->data, l2pkt, m->data_len);
					...
					if (interface->available.head->size > 0)
					{
	...
	#endif /* TAP_INTERFACE */
	#endif /* ENCAP_DIVERT */
						//printf("service_interface: divert port:%d\n",interface->available.head->divert_port_number);
						...
						memcpy (&(interface->available.head->src_mac_addr [0]),
								&(interface->available.head->data [MAC_ADDR_SIZE]),
								MAC_ADDR_SIZE);

						if( (0x81==interface->available.head->data[START_OF_FRAME_TYPE]) && (0x00==interface->available.head->data [START_OF_FRAME_TYPE+1]) )
						{
							interface->available.head->frame_type = (
								((int) (((int) interface->available.head->data [START_OF_FRAME_TYPE+4]) * 256)) +
								((int) ((int) interface->available.head->data [START_OF_FRAME_TYPE+5])));
						}
						else
						{
							interface->available.head->frame_type = (
						...
						if (interface->incoming.tail)
						{
							...
							interface->incoming.head = interface->incoming.tail = interface->available.head;
						...
						interface->incoming.size++;
						scheduler.interface_data++;
						//printf("ll_support.c 1965: incoming.head=%x\n", interface->incoming.head);
					}
				}
				else
				{
					printf("interface->incoming.head is null\n");
					rte_pktmbuf_free(m);
				} 
#endif	
			}
