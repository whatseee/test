...
extern FILE *fp_log;
extern FILE *fp_cksum;     //YGX
//extern FILE *fp_timer;


#define LEN_OF_TIME_STR          128

void log_msg(char* fmt, ...);
//static int month_days(unsigned int month, unsigned int year);
//void rtc_time_to_tm(unsigned long time, struct rtc_time *tm);
void time_print(char *timestring);
void generate_bak_log(void);
extern int num_of_log_record;
#define MAX_NUM_OF_LOG_RECORD    30
...
typedef struct _tp_socket
{
    ...
    int nl_protocol_id;					/* NP, IPV4, IPV6 */													/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE,\D4\DAscps_socket\D6н\F8\D0\D0\C1˳\F5ʼ\BB\AF*/
    ...
    scps_np_addr my_ipv4_addr, his_ipv4_addr;		/* internet address of peer */								/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    struct ipv6_addr my_ipv6_addr, his_ipv6_addr;	/* internet address of peer */								/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    u_short myport, hisport;			/* tp ports for this connection */										/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int np_size;						/* Size of np header for this connection */								/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE(\D2\F2Ϊ\B3\CC\D0\F2\D6д\D3\C0\B4\BE\CDû\D3ж\A8\D2\E5\BA\EA USESCPSNPXXX)?*/
    int sp_size;						/* Size of sp header for this connection */
    short nh_off;						/* Offset of the start of the np header in an mbuff */					/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE(\D2\F2Ϊ\B3\CC\D0\F2\D6д\D3\C0\B4\BE\CDû\D3ж\A8\D2\E5\BA\EA USESCPSNPXXX)?*/
    ...
    route *rt_route_def;				/* Default routing structure */											/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int s_errno;						/* Error indication consumed by app */									/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    scps_np_rqts np_rqts;				/* Network layer requirements structure */                              /*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶε\C4nl_protocol\D7ֶ\CE(\D2\F2Ϊ\B3\CC\D0\F2\D6д\D3\C0\B4\BE\CDû\D3ж\A8\D2\E5\BA\EA NL_DEFAULT_IPV4)?*/
     ...
     */
    int display_now;
    ip_template ip_templ;		/* Network layer header template */												/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    scps_np_template np_templ;	/* Network layer header template */												/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
#ifdef XXX
    path_template pa_templ;		/* Network layer header template */												/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
#endif/* XXX */
#ifdef SCPSSP
...
#ifdef GATEWAY
    struct _tp_socket *peer_socket;	/* Pointer for the peer socket */											/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int gateway_flags;				/* What should the peer do */												/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    uint32_t fin_seqnum;																						/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int32_t divert_port;																						/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int32_t gateway_lan_or_wan;																					/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    unsigned rel_seq_num_urg_ptr;	/* offet from init_seq_num of urg data */
    int gateway_runt_ctr;
    int gateway_runt_ack_ctr;
    int gateway_fairness_ctr;																					/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    int gateway_next_hop;			/* USED WITH DIVERT_B_RAWIP ONLY */											/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
#ifdef GATEWAY_DUAL_INTERFACE
    int gateway_layering;																						/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    ...
    unsigned char DSCP;																							/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    unsigned char strict_DSCP;																					/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    unsigned short protocol_id;																					/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    short state;							/* connection state */
    short state_prev;						/* connection state prior to above */

	 short state_last;
	 uint32_t recv_packet_num;
	 uint32_t KA_L_counter;
	 uint32_t KA_W_counter;
    ...
    uint32_t high_hole_seq;					/* Sequence number of the highest SNACKed byte during a congestion epoch. */
    uint32_t high_seq;						/* Highest sequence number sent when we transition into fast retransmit. NewReno. */
    uint32_t high_congestion_seq;			/* The highest sequence number sent during a congestion epoch.  On leaving a congestion epoch we don't get snd_cwnd credit for acks from packets sent during the epoch. */
    uint32_t pkts_ack_in_epoch;				/* This is the number of packets that were acked during a congestion epoch */
    ...
    /* 2 = every 2nd segment */
    short int advance_hole;					/* Used to re-snack the hole at snd_una multiple times. */
    short int mfx_snd_una;					/* used to mfx the packet at send_una 0 = Dont use this feature # = number of times to mfx the pkt at snd_una */
...
#ifdef TAP_INTERFACE
    unsigned char src_mac_addr [6];																			/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
    unsigned char dst_mac_addr [6];																			/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
	 unsigned short TPID;				//802.1Q--Tag Protocol Identifier
 	 unsigned short TCI;					//802.1Q--Tag Control Information
    unsigned short frame_type;																				/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
	 unsigned int mpls_r[MAX_MPLS_DEPTH];
	 unsigned int mpls_depth_r;
	 unsigned int mpls_s[MAX_MPLS_DEPTH];
	 unsigned int mpls_depth_s;
    int recv_tap_if;																						/*tp_Common\D6\D0δ\B3\F5ʼ\BB\AF\B8\C3\D7ֶ\CE*/
#endif /* TAP_INTERFACE */
    ...
    //unsigned char father_function[50];
    uint32_t father_line;
    void *bbr;
    uint32_t gateway_move_num;
    uint32_t pre_time;
    int pkg_cnt;
    uint32_t next_send_time;
    uint32_t interval_send_time;
    float fpacing_rate;
    uint32_t packet_loss;
    uint32_t is_fast_retransmit;
    uint32_t pre_dup_cnt;
	int remote_flag;
}tp_Socket;
...
#define ALPHA	60 //3 /* 5 */	/* Bigger = more likely to increase congestion window */
#define BETA	100//5 /* 7 */	/* Bigger = less likely to decrease congestion window */
#define GAMMA	80 //4				/* Bigger = come out of exponential mode later */
...
void tp_TFKeep (tp_Socket * s);
void tp_TFTimeWT (tp_Socket * s);
void tp_TFKeepAlive_L (tp_Socket * s);
void tp_TFKeepAlive_W (tp_Socket * s);
...
int tp_OutSeq (tp_Socket * s, tp_Header * tp, int tp_len,
	       byte * dp, int dp_len);
int tp_OutSeq_loop (tp_Socket * s, tp_Header * tp, int tp_len,
	       byte * dp, int dp_len);
...
struct mbuff *tp_BuildHdr_KA (tp_Socket * s, struct mbuff *mbuffer, int push);
