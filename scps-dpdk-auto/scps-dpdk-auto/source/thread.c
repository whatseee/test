...
init_scheduler()
{
	...
	scheduler.switch_time = 0;	/* Really should be current time, but
					 * I'm lazy */
	scheduler.timers.queue.head = NULL;
	scheduler.timers.queue.tail = NULL;
	scheduler.timers.expired_queue.head = NULL;
                ...
                create_thread(void (*program) (void),...)
{
	...
	threadp = (struct threads *) my_malloc(sizeof(struct threads));
	threadp->pid = ++scheduler.last_alloc;
	threadp->status = Created;
	threadp->function = program;
	threadp->stack_size = 1024;
	threadp->stack = (char *) my_malloc(THREAD_SWAP_STACK_SIZE);
