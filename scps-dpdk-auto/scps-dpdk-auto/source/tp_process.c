...
void tp_ProcessAck (tp_Socket * s, tp_Header * tp, int data_len)
{
  ...
  ST_BBR *bbr = s->bbr;
  mytime.tv_sec = 0;
  static temptime_all = 0;
  static pkg_cnt_all = 0;

	if( (!s) || (!tp) )
	{
		printf("Gateway-->Function tp_ProcessAck: s=%d, tp=%d!\n", (int)s, (int)tp );
		fflush(stdout);
		return;
	}
#if 0
  ...
  if ((((ntohs (tp->window) << s->snd_scale) < s->maxseg) ||
	...
	if ((ntohl(tp->seqnum) == (s->acknum -1)) && (data_len == 0))
	{
		//printf("Here comes a KA packet1111111111111111\n");
		//fflush(stdout);
  ...
  if(s->remote_flag == 0)
		{
			if(s->is_bbr_sk == 10000)
			{
				curbbr = bbr;
				// if(s->dup_ack_cnt != 0)
				// 	s->pre_dup_cnt = s->dup_ack_cnt;
				if(diff == 0)
				{
					s->pkg_cnt += 1400;
				}
				...
				if(s->pre_time == 0)
				{
					s->pre_time = get_time_us();
					bbr->cwnd = 4;
				}
				uint8_t flagbbr = 0;
				uint32_t temptime = (get_time_us() - s->pre_time);
				if( temptime >= 5000 )//|| bbr->hole == 1
				{
					if(s->pre_dup_cnt > 200 && s->pre_dup_cnt != 0 && s->dup_ack_cnt == 0)
					{
						s->pkg_cnt -= s->pre_dup_cnt * 1400;
						if(s->pkg_cnt < 0)
							s->pkg_cnt = 0;
						
					}
					//s->packet_loss += s->pre_dup_cnt;
					float speed = (s->pkg_cnt) /(temptime * 1.0);
					// if(speed > 10)
					// 	printf("speed***********************************=%f\n",speed);
					s->pre_time = get_time_us();
					// if(speed > 22 && (s->pre_dup_cnt != 0 || s->dup_ack_cnt != 0 || s->packet_loss != 0) )
					// {
					// 	s->pkg_cnt = 5*1400;
					// }
					flagbbr = 1;
					s->interval_send_time = s->ts_recent;
					bbr->srtt = 0;
				}
				if(flagbbr == 1)
				{

					temptime_all += temptime;
					pkg_cnt_all += s->pkg_cnt;
				}
				if(temptime_all >= 1000000)
				{
					bbr->is_long_bw = 1;
					bbr->long_bw = (pkg_cnt_all* 1.0)/(temptime_all * 1.0);
					temptime_all = 0;
					pkg_cnt_all = 0;
				}
				...
				if(bbr->srtt == 0)
					bbr->srtt = bbr->rtt;
				bbr->srtt = 0.8 * bbr->srtt + 0.2 * bbr->rtt;
				if(s->interval_us != 0)
				{
					bbr->interval_us = temptime;
				}
				else
				{
					bbr->interval_us = temptime;
				}
				if(bbr->cwndUsed >= diff)
				{
					bbr->cwndUsed -= diff;
				}
				else
				...
				bbr->deliver = s->pkg_cnt;
				bbr->rtt = s->brtt;
				update_min_rtt(s,bbr);
				if(flagbbr)
				{
					bbr_main(s,bbr,s->pkg_cnt,flagbbr);
					
					// printf("s->pkg_cnt:%d\n",s->pkg_cnt);
					// printf("s->dup_ack_cnt:%d\n",s->dup_ack_cnt);
					// printf("s->pre_ack_cnt:%d\n",s->pre_dup_cnt);
...
//	 logEventv(s, xplot, "%d %u %u %d\n", s->snd_prevcwnd, s->seqsent, flippedack, s->snd_cwnd);
      ...
      if (s->cong_algorithm == VEGAS_CONGESTION_CONTROL)
	{
	  ...
	  ((s->write) &&
	   ((s->send_buff->max_size - (s->send_buff->data_size + s->app_sbuff->size)) >= s->write)))
	   //((s->app_sbuff->max_size - s->app_sbuff->size) >= s->write)))

	{
	  ...
	  ((s->send_buff->max_size - (s->send_buff->data_size + s->app_sbuff->size)) >= scheduler.sockets[s->sockid].write)))
	  //((s->app_sbuff->max_size - s->app_sbuff->size) >= scheduler.sockets[s->sockid].write)))
	{
	  s->thread->status = Ready;
	  scheduler.num_runable++;
	  ...
	  if (!(s->funct_flags & FUNCT_HIGH_SEQ)
	      ...
	      if (s->cong_algorithm == VJ_CONGESTION_CONTROL)
		{
		  ...
		  s->snd_prevcwnd = min(s->snd_prevcwnd, s->seqsent - s->snduna);
                  ...
                  s->snd_ssthresh = max ((s->maxdata << 1), (s->snd_prevcwnd >> 1));
				//s->snd_ssthresh = 196608;
		...
		if(s->maxdata!=0)
		{
			...
			{
				s->snd_prevcwnd = (((int) ((s->snd_ssthresh) / s->maxdata))*s->maxdata);
		  	} 
			else 
			{
            s->snd_prevcwnd = (((int) ((s->snd_ssthresh) / s->maxdata))*s->maxdata) + s->maxdata;
		  	}
		}
...
#define  LINUX_STYLE_EPOCH 1
 ...
 if (s->gateway_flags & GATEWAY_HAS_RUNT) {
	...
	//if (sys_memory.clust_in_use + 100 > sys_memory.fclist.max_elements)	//modified on 20131124
	{
  ...
  if (flippedack == s->seqsent)
    {
      clear_timer (s->otimers[Rexmit], 1);
		...
		if( (ntohs(tp->window)<<s->snd_scale) < 1536 )
		{
			mytime.tv_sec = 0;
...
void tp_ProcessData (tp_Socket * s, tp_Header * tp, byte * data, int len)
{
	int x;
  	...
  	int ack_now = 0;
  	uint32_t flippedseq;
  	uint32_t length_to_trim = 0;
  	uint32_t amount_to_ack = 0;
  	struct timeval mytime = {0, s->ACKDELAY};

	if( (!s) || (!tp) || (!data) )
	{
		printf("Gateway-->Function tp_ProcessData: s=%d, tp=%d, data=%d!\n", (int)s, (int)tp, (int)data );
		fflush(stdout);
		return;
	}
...
#define MAX_UNACKED_SEGS 2
	...
	if ((len != 0) && (sys_memory.clust_in_use + 100 >= sys_memory.fclist.max_elements))		//modified on 20131124
	{		
		printf("tp_process.c1: max_element = %d!\n", sys_memory.fclist.max_elements);
     	return; 
  	}
      ...
      s->ack_delay++;
      s->unacked_segs++;
      if ((s->ack_freq == 1) || ((s->ack_freq == 2) && (s->unacked_segs >= MAX_UNACKED_SEGS)))
			ack_now = 1;
      else
		{
   ...
   * acknum multiple times.
	...
	if ((flippedseq == s->acknum -1) && (len == 1))
	{
		//printf("Here comes a KA packet2222222222222222\n");
		//fflush(stdout);
		ack_now == 1;
		goto ACKNOW;
	}
      ...
      if ((flippedseq < uwe)||((flippedseq > uwe)&&(flippedseq - uwe > 0x80000000)))
		{
	  		/* is any of the packet lower than uwe? */
	  		//if (((s->state == tp_StateESTAB) && flippedseq < s->acknum))
			//YGX 20140209

#if 0
			//if((flippedseq > uwe) && (flippedseq - uwe > 0x80000000))
				//printf("tp_Processdata1111: flippedseq = %u, uwe = %u, cwnd = %u, awnd = %u!!!\n", flippedseq, uwe, s->snd_cwnd, s->snd_awnd);
			if((flippedseq < s->acknum) && (s->acknum - flippedseq > 0x80000000))
				printf("tp_Processdata2222: flippedseq = %u, acknum = %u!!!\n", flippedseq, s->acknum);
			if((flippedseq > s->acknum) && (flippedseq - s->acknum > 0x80000000))
				printf("tp_Processdata3333: flippedseq = %u, acknum = %u!!!\n", flippedseq, s->acknum);
			fflush(stdout);
#endif

			if (((flippedseq < s->acknum) && (s->acknum - flippedseq < 0x20000000)) || ((flippedseq > s->acknum) && (flippedseq - s->acknum > 0x80000000)) )		//modified by cti on 20140209 滞后
	    		{
	      	/* is it a rexmit? */
	      	...
	      	if (s->ack_freq)
				...
				//added by cti on 2013.12.27
				if((flippedseq + len > s->acknum) && (flippedseq + len - s->acknum < 0x20000000)) 
				{
					dp += (s->acknum - flippedseq);
	      		len -= (s->acknum - flippedseq);
				}
	  		...
	  		else if((flippedseq > s->acknum)&&(flippedseq - s->acknum < 0x20000000))   //超前
	    		{
				 if ((len > 0) || (flags & tp_FlagFIN))
				{
		   		...
		   		* --KS
		   			*/
		  			//if ((ntohl (tp->seqnum) + len > s->acknum) || ((flags & tp_FlagFIN) && (ntohl (tp->seqnum) == s->acknum)))	//Deleted by cti on 20131213!
		    			{
		   		...
		   		* There needs to be a better way...
		  			...
		  			if (((s->Out_Seq->last) && SEQ_GEQ ((s->Out_Seq->last->m_seq + s->Out_Seq->last->m_plen), s->lastuwe)) || (flags & tp_FlagFIN))
		    			{
			...
			{
				//printf("tp_Processdata9999: flippedseq = %u, acknum = %u!!!\n", flippedseq, s->acknum);
				//fflush(stdout);
				if ((len > 0) || (flags & tp_FlagFIN))
				{
		  			/*
		   		...
		   		* --KS
		  			...
		  			//if ((ntohl (tp->seqnum) + len > s->acknum) || ((flags & tp_FlagFIN) && (ntohl (tp->seqnum) == s->acknum)))	//Deleted by cti on 20131213!
		    			{
		      		if (tp_OutSeq_loop (s, tp, x, dp, len) < 1)
							printf ("tp_OutSeq failed: ");
		    			}
...
#if 0
			if (flippedseq < s->acknum)		//modified by cti on 20131213
	    		{
	      	/* is it a rexmit? */
	      	//dp += (s->acknum - flippedseq);
	      	//len -= (s->acknum - flippedseq);
	      	...
	      	if (s->ack_freq)
	    		...
	    		{
#if 0
				if ((flippedseq - s->acknum) > 0x80000000)
				{
					printf("(flippedseq = %u, s->acknum = %u\n", flippedseq, s->acknum);
					if (s->ack_freq)
					{
#ifndef MPF
		  				ack_now = 1;	/* let him know */
					...
					if((flippedseq + len - 0xffffffff) > s->acknum)
					{
						dp += (s->acknum + 0xffffffff - flippedseq);
	      			len -= (s->acknum + 0xffffffff - flippedseq);
					}
				}
	      			/* 
      ...
      if ((mbuffer = alloc_mbuff (MT_HEADER)))
	{
	  ...
	  if ( (s) && (s->peer_socket) )
	    {
 		...
 		if (s->Out_Seq->start && s->Out_Seq->last)
		{		
			//printf("s->Out_Seq->start->m_seq: %u, s->Out_Seq->last->m_seq: %u, s->acknum: %u\n", s->Out_Seq->start->m_seq,s->Out_Seq->last->m_seq, s->acknum);
			//fflush(stdout);
		}
	  if ((s->Out_Seq->start) &&
	  ...
	  {			/* Here */
	      ...
	      mbuffer = deq_mbuff (s->Out_Seq);
			//printf("s->Out_Seq->start->m_seq: %u, s->acknum: %u\n", mbuffer->m_seq, s->acknum);
			//fflush(stdout);
			if(!mbuffer)
			{
				printf("Gateway-->Function tp_ProcessData: In line 2100, deq_mbuff return NULL!\n");
				fflush(stdout);
				return;
			}

			if(mbuffer->m_plen < 0)
				printf("processData Error: File %s, Line %d, mbuffer->m_plen = %d!\n", __FILE__, __LINE__, mbuffer->m_plen);
         ...
         * ELSE on the out-of-sequence queue.
               		*/
	      flags =  ((tp_Header *) (mbuffer->m_pktdat))->flags;
#if 0
...
#endif
#if 1
			if (mbuffer->m_seq + mbuffer->m_plen < s->acknum)
			{
#ifdef OPT_SNACK1
	      	if (s->capabilities & CAP_SNACK)
				{
   	...
   	if (ack_now)
		{
			...
			s->flags = tp_FlagACK;
			//printf("tp_process.c 2782,tp_BuildHdr\n");
	  		if ((mbuffer = tp_BuildHdr (s, NULL, 0)))
	    		{	      
#ifdef GATEWAY
	      	if ((s) && (s->gateway_flags & GATEWAY_PEER_WIN_NOT_OPENED))
				{
#ifdef GATEWAY_DEBUG
	      	...
	      	if (tp_NewSend (s, mbuffer, false) > s->np_size)
				{
		  			s->unacked_segs = 0;
...
void Process_KA (tp_Socket * s)
{
	...
	if ((mbuffer = alloc_mbuff (MT_HEADER)))
	{
		if ((mbuffer = tp_BuildHdr_KA (s, NULL, 0)))
		{	      
			if (tp_NewSend (s, mbuffer, false) > s->np_size)
			{
				//printf("sockid %d Send a KA packet.............\n", s->sockid);
				fflush(stdout);
			  	s->unacked_segs = 0;
			  	s->lastuwe = s->acknum + s->rcvwin;
			  	s->lastack = s->acknum;
...
Process_EOR (tp_Socket * s, struct mbuff *mbuffer, int Out_Seq)
{
  ...
  Rec_Bound =
    (struct record_boundary *) my_malloc (sizeof (struct record_boundary));
