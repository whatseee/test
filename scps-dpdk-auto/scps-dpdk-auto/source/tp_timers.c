 ...
 */
unsigned int vegas_cnt = 0;
unsigned int vegas_array[8]={0,0,0,0,0,0,0,0};
...
tp_TFRetransmit (tp_Socket * s)
{
  ...
  if (s->state == tp_StateCLOSED)
	...
	if( (s->state==tp_StateSYNSENT) && (s->timeout<s->TIMEOUT) )
	{
#ifdef GATEWAY
      ...
      if (s->state == tp_StateTIMEWT)
	{
	  ...
	  tp_Unthread (s);
          return;
	       ...
	       (s->send_buff->snd_una) && (s->send_buff->snd_una->pep_m_next))
	{
	  /*
	  ...
	  if ((((tp_Socket *)
		scheduler.sockets[s->sockid].ptr)->thread->status == Blocked)
	      ...
	      ((s->send_buff->max_size - s->send_buff->b_size) >= scheduler.sockets[s->sockid].write))
...
/*
	      ...
	      mytime.tv_usec = max (s->RTOMIN, mytime.tv_usec) << s->t_rxtshift;
	      mytime.tv_usec = min (mytime.tv_usec, s->RTOMAX);
*/
			if( (s->state==tp_StateSYNSENT) && (s->timeout==(s->TIMEOUT-1)) )
			{
				mytime.tv_usec = 4000000;
			}
			else
			{
	      			if( s->timeout >= (s->TIMEOUT-1)  )
...
printf("Gateway-->tp_TFRetransmit: the last mytime.tv_usec=%d!\n", mytime.tv_usec);
fflush(stdout);
...
*/
  ...
  if ( (s->otimers[Rexmit]->set) || (s->otimers[Rexmit]->expired)) {
	return;
  	...
  	//if ((sys_memory.clust_in_use + 10 >= sys_memory.fclist.max_size) || (!(s->send_buff->send))) 
	if ((sys_memory.clust_in_use + 100 >= sys_memory.fclist.max_elements) || (!(s->send_buff->send)))	//modified on 20131124
	{
     	mytime.tv_sec = 0;
  ...
  if ((buf) && (buf->m_ext.len))
    {
      ...
      mbuffer = alloc_mbuff (MT_HEADER);
		...
		if(!mbuffer)
		{
			printf("Gateway-->Fuction tp_TFPersist: alloc_mbuff return NULL!\n");
			fflush(stdout);
			return;
		}
...
#endif /* GLOBAL_VEGAS */
    	{
   	//cwinsegs = min ((s->snd_prevcwnd / s->maxseg), max ((s->snd_prevcwnd-(s->snduna-s->rtseq))/s->maxseg, 1));
		cwinsegs = min ((s->snd_prevcwnd / s->maxseg), max ((s->rtseq-s->snduna)/s->maxseg, 1));
...
/////////////////////////////////////////////////////////////
#if 0
			printf("tp_TFVegas %d: vegas_array_1=%12d,%12d,%12d,%12d!\n", vegas_cnt,vegas_array[0],vegas_array[1],vegas_array[2],vegas_array[3]);

			for(i=0; i<4; i++)
			{
				if(vegas_array[i]==0)
				{
					vegas_array[i]=s->rttcur;
					break;
				}
			}

			printf("tp_TFVegas %d: vegas_array_2=%12d,%12d,%12d,%12d!\n", vegas_cnt,vegas_array[0],vegas_array[1],vegas_array[2],vegas_array[3]);

			if(i >= 4)
			{
				for(i=0; i<3; i++)
				{
					vegas_array[i] = vegas_array[i+1];
				}
				vegas_array[3] = s->rttcur;

				printf("tp_TFVegas %d: vegas_array_3=%12d,%12d,%12d,%12d!\n", vegas_cnt,vegas_array[0],vegas_array[1],vegas_array[2],vegas_array[3]);

				for(i=0; i<3; i++)
				{
					if(vegas_array[i+1] > vegas_array[i])
					{
					}
					else
						sign1 = 1;                           
				...
				printf("tp_TFVegas %d: vegas_array_sign1=%d!\n", vegas_cnt, sign1);

				if(sign1 == 0)
				{
					if( ((vegas_array[3]-vegas_array[2])>(vegas_array[2]-vegas_array[1])) &&
				 		((vegas_array[2]-vegas_array[1])>(vegas_array[1]-vegas_array[0]))
			     			)
						sign2 = 1;

					printf("tp_TFVegas %d: vegas_array_sign2=%d, cha1=%d, cha2=%d, cha3=%d!\n", vegas_cnt, sign2, (vegas_array[3]-vegas_array[2]), (vegas_array[2]-vegas_array[1]), (vegas_array[1]-vegas_array[0]) );
				}				
			}		

			if(sign1 == 1) //rtt mei you lian xv zeng zhang!
			{
	      	new_cwnd += s->maxdata;
	      	/* s->snd_cwnd += s->maxdata; */
	      	...
	      	switch ( VEGAS_BACKOFF ) 
				{
	      		case 0: /* Experimental */
		 				...
		 				*/
						//new_cwnd -= min((new_cwnd/2), (((diff-s->VEGAS_BETA)*s->maxdata)/2));
						new_cwnd = (new_cwnd * 7 / 8);
						s->sockFlags &= ~TF_CC_LINEAR;       //added bye CTI!
						break;
...
#undef VEGAS_BACKOFF 
      ...
      else/* exponential */
		{
      	if ( (s->VEGAS_SS == 0) || ( (s->VEGAS_SS != 0) && (new_cwnd == s->snd_ssthresh) ))
			...
			//printf("tp_TFVegas:sock_id=%d, snd_ssthresh=%d!!!\n",s->sockid, s->snd_ssthresh);
...
tp_TFRate (route *r)
{
  ...
  while (r) {
    if (((route *) r)->current_credit < (int)(((route *) r)->max_credit) )
      {
  	if (((route *) r)->bytes_per_interval == 0) {
  		...
  		if ((((route *) r)->shifted_rate_bucket) >
  			...
  			((route *) r)->current_credit += (int)temp;
...
/*	printf ("Current_credit (%lu) double_bucket (%d)\n", 
        ...
        if ((((route *) r)->current_credit) > (int)(((route *) r)->max_credit))
  	(((route *) r)->current_credit) = (int)(((route *) r)->max_credit);
      }
...
tp_TFBERecv (tp_Socket * s)
{
  ...
  mbuffer = deq_mbuff (s->Out_Seq);
	...
	if(!mbuffer)
	{
		printf("Gateway-->Fuction tp_TFBERecv: deq_mbuff return NULL!\n");
		fflush(stdout);
		return;
	}
...
{
	struct timeval mytime;
  	if ( (s) && (s->state >= tp_StateSYNSENT) && (s->state <=tp_StateLASTACK)) 
	{     
		s->sockFlags |= SOCK_ACKNOW;
  	}
...
{
	struct timeval mytime;
	tp_Socket *peer_s = NULL;

	if(!s)
		return;

  	if ( (s->state >= tp_StateSYNSENT) && (s->state <=tp_StateLASTACK)) 
	{     
		if(s->recv_packet_num > 0)
		{
			//if(debug[2]==1)
				//printf("TFKeepAlive1-->sockid=%d,state=%d,myaddr=0x%x,hisaddr=0x%x,myport=%d,hisport=%d,recv=%d!\n",s->sockid, s->state, s->my_ipv4_addr, s->his_ipv4_addr, s->myport, s->hisport, s->recv_packet_num);
		...
		else
		{
			...
			{
				//if(debug[2]==1)
					//printf("TFKeepAlive2-->sockid=%d,state=%d,myaddr=0x%x,hisaddr=0x%x,myport=%d,hisport=%d!\n",s->sockid, s->state, s->my_ipv4_addr, s->his_ipv4_addr, s->myport, s->hisport);

				peer_s = s->peer_socket;

#ifdef GATEWAY
				if((peer_s) && (peer_s->peer_socket==s))
				{
			...
			else
			{
				...
				//if(debug[2]==1)
					//printf("TFKeepAlive3-->sockid=%d,state=%d,myaddr=0x%x,hisaddr=0x%x,myport=%d,hisport=%d,KA_count=%d!\n",s->sockid, s->state, s->my_ipv4_addr, s->his_ipv4_addr, s->myport, s->hisport, s->KA_counter);
			//}
		}		
  	}
...
{
	//printf("tp_TFKeepAlive_W tp_TFKeepAlive_W tp_TFKeepAlive_W \n");
	fflush(stdout);
	struct timeval mytime;
	tp_Socket *peer_s = NULL;
	...
	{     
		...
		{
#if 1
			if(s->KA_W_counter >= 50)
			{
				//if(debug[2]==1)
				printf("TFKeepAlive_W called 5 times with no ack, Abort the socket!\n");
				fflush(stdout);
				peer_s = s->peer_socket;
			...
			else
			{
				...
				//if(debug[2]==1)
					//printf("TFKeepAlive3-->sockid=%d,state=%d,myaddr=0x%x,hisaddr=0x%x,myport=%d,hisport=%d,KA_W_count=%d!\n",s->sockid, s->state, s->my_ipv4_addr, s->his_ipv4_addr, s->myport, s->hisport, s->KA_W_counter);
			}
		}		
  	}
...
/*
	...
	if(debug[1]==1)
	{
		sprintf(buf, "TFKeep-> sockid=%d,state=%d,myaddr=0x%x,hisaddr=0x%x,myport=%d,hisport=%d!\n",s->sockid, s->state, s->my_ipv4_addr, s->his_ipv4_addr, s->myport, s->hisport);
		if(fp_timer)
			fwrite(buf, 1, strlen(buf), fp_timer);
	}
*/

	if((s->state<tp_StateESTAB) && (s->state!=tp_StateLISTEN))
	{
		peer_s = s->peer_socket;
	...
	if((s->state>tp_StateESTAB) && (s->state!=tp_StateTIMEWT) && (s->state!=tp_StateCLOSED))
	{
		if(s->state_last == s->state)
		{
			peer_s = s->peer_socket;
			...
			s->state_last = s->state;
	}

	mytime.tv_sec = 60;
	mytime.tv_usec = 0;
	set_timer(&mytime, s->otimers[Keep], 1);
}
