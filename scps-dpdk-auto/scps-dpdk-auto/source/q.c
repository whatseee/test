 ...
 * This routine must use `cfree' rather than `my_free' because queue elements
...
q_create ()			/* Modified 6/7/91 by N. Schult */
{
  ...
  q = (queue *) my_malloc (sizeof (queue));

  if(!q)
     {
		printf("Gateway-->Function q_create: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
     }
...
q_addh (q, elt)
     ...
     register caddr_t elt;
{
  register q_elt *new = (q_elt *) my_malloc (sizeof (q_elt));

  if(!new)
     {
		printf("Gateway-->Function q_addh: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
     }
...
/*  register q_elt *new = (q_elt *)calloc(1, sizeof(q_elt)); */
  ...
  if(!new)
     {
		printf("Gateway-->Function q_addt: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
     }
     ...
     register caddr_t prev, new;
{
  ...
  newqe = (q_elt *) my_malloc (sizeof (q_elt));
  if(!newqe)
     {
		printf("Gateway-->Function q_adda: my_malloc return NULL\n");
		fflush(stdout);
		return NULL;
     }
  newqe->qe_data = new;
