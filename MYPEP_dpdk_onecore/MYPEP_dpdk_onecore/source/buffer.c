...
struct _sys_memory sys_memory =
{
  ...
  0,				/* Number of clusters actually my_malloc'd */
...
if ( (q->start == q->last) && (q->start == NULL) )
    {
      ...
      buf->m_prev = buf->pep_m_next = NULL;
  ...
  else
    {
		...
		{
			printf("Gateway-->Function enq_mbuff: q->last is NULL!\n");
			fflush(stdout);
		}
    }
  buf->parent = q;
 ...
 * here to walk through the m_next chain for a header mbuff and my_free that up,
...
kill_bchain (struct mbuff *buff_head)
{
  ...
  if ((buff_head) && (buff_head->m_prev))
    buff_head->m_prev->pep_m_next = NULL;

  while (buff_head)
    {
      if (buff_head->pep_m_next)
	{
	  buff_head = buff_head->pep_m_next;
	  free_mbuff (buff_head->m_prev);
	  /* We would my_free-up a sub-chain here - see above */
	}
      else
	{
...
/*		printf ("Tried to Free a cluster and could not find it\n"); */
	  ...
	  if ( (cluster)  && (!(enq_mclus (cluster, &(sys_memory.fclist)))))
	    {
              //sys_memory.clust_in_use--;       //This line should be deleted!
              clust_mem_map.clust_list [cluster->cluster_id].used = -1;
	      my_free ((char *) cluster);
...
free_mbuff (struct mbuff *buf)
{
  ...
  /* If this mbuff has a cluster associated with it, my_free it first */
  if (buf->m_flags & M_EXT)
      ...
      if (size > SMCLBYTES)
	{
	   ...
	   * we will try and my_free one more cluster than we've
 ...
 * maximum buffer space available, a new mbuff is my_malloc'd. Note: This
 * routine currently assumes that buffer space is assigned in fixed sized
 * chunks, so if there is buffer space available, at worst, another my_malloc
...
alloc_mbuff (int type)
{
  	...
  	if (sys_memory.fblist.b_size == 0)
   	...
   	if (sys_memory.mbuff_created < MAX_MBUFFS)
		{
	  		if (!(temp = (struct mbuff *) my_malloc (sizeof (struct mbuff)))) 
			{
	      	return ((struct mbuff *) NULL);
          		}
//deleted by cti
/*	
	  		if (!(temp->m_data = (char *) my_malloc (MBLEN * sizeof (char))))
	    		{
	      	my_free ((char *) temp);
...
*/
			...
			memset (temp, 0, (sizeof (struct mbuff))); //modified by CTI!
	  		temp->m_type = type;
  	...
  	if (!temp) 
	{
  	...
  	memset (temp, 0, (sizeof (struct mbuff) - MHLEN));
  	temp->m_type = type;
  	temp->pep_m_next = temp->m_prev = NULL;
 ...
 * maximum cluster space available, a new mbcluster is my_malloc'd. Note: This
...
alloc_mbclus (int reserve_pool)
{
  	...
  	//if (((!cluster_check) && (sys_memory.clust_in_use + 5 < sys_memory.fclist.max_size))) 
	if (((!cluster_check) && (sys_memory.clust_in_use + 5 < sys_memory.fclist.max_elements))) 	//modified on 20131124
	{ 
  	} 
	else 
	{
      //if ((reserve_pool) &&(sys_memory.clust_in_use + 20 > sys_memory.fclist.max_size)) 
		if ((reserve_pool) &&(sys_memory.clust_in_use + 100 > sys_memory.fclist.max_elements))	//modified on 20131124
		{
#ifdef MEMORY_DEBUG
  ...
  if (sys_memory.fclist.num_elements == 0)
    {
      //if (sys_memory.clust_created < sys_memory.fclist.max_size)
		if (sys_memory.clust_created < sys_memory.fclist.max_elements)
	{
	  if (!(temp = (struct mbcluster *) my_malloc (sizeof (struct mbcluster))))
...
/*          registerBlock (temp, 10, __FILE__,  __LINE__); */
  ...
  else /* There is at least one element on the my_free cluster list. */
    {
      if (sys_memory.fclist.start == NULL)
	{
	  printf("Thought there were elements on the my_free list but there are none!\n");
	}
      temp = deq_mclus (&(sys_memory.fclist));
      if (!temp) {
         printf ("%s %d 1 tried to dequeue a clust off the my_free list, but non was there \n", __FILE__, __LINE__);
...
alloc_hole_element (void)
{
  ...
  if (sys_memory.hole_list)
	...
	sys_memory.hole_list->prev = 0x0;
      ...
      ((temp = (struct _hole_element *) my_malloc (sizeof (struct
							_hole_element))))
    {
          ...
          uint32_t snack_delay)
{
  ...
  hole = alloc_hole_element ();
	if(!hole)
	{
		printf("Gateway-->Function add_hole: alloc_hole_element return NULL!\n");
		fflush(stdout);
		return list;
	}
  ...
  if (len)
	...
	printf("Trimming hole to (%lu:%lu) [%lu]\n",
      ...
      while ((((int) len) > 0) && (hole->hole_end->pep_m_next))
	{
	  hole->hole_end = hole->hole_end->pep_m_next;
	  len -= hole->hole_end->m_ext.len;
	}
      if (((int) len) > 0)
	hole->length -= len;
  ...
  if (hole->length)
	...
	{
    	temp = list;
		printf("Gateway-->Function add_hole: mem leak at the last!\n");
		fflush(stdout);
		free_hole_element (hole);  //added by cti!
	}
...
/* Remove 0 or more holes below seqnum - this doesn't my_free the
      ...
      if ((list) && SEQ_GT (seqnum, list->hole_start_seq))
	{
	  ...
	  while ((list->hole_start != list->hole_end) &&
		 SEQ_LT (list->hole_start->m_seq + list->hole_start->m_ext.len,
			 seqnum))
	    list->hole_start = list->hole_start->pep_m_next;
	}
    }				/* if (list) */
  return (list);
}
	    ...
	    int len, int offset, int maxseg)
{
  ...
  while ((len > 0) && (temp_write_head))
	...
	size = maxseg - chain->run_length;
		...
		if(temp_write_off < 0)
			printf("cb_cpdatin: temp_write_off=%d < 0!\n", temp_write_off);
		else
      	memcpy ((void *) (temp_write_head->c_data + temp_write_off), dp, size);
      len -= size;
      dp += size;
      temp_write_off += size;
...
{
  struct _buffer *buffer = (struct _buffer *) my_malloc (sizeof (struct _buffer));
...
chain_init (uint32_t max_size)
{
  ...
  chain = (struct _cl_chain *) my_malloc (sizeof (struct _cl_chain));
  ...
  if ((temp_clust = alloc_mbclus (1)))
    {
      ...
      /* 
       * If we can't get the temporary cluster, my_free the chain and bail.
       */
      my_free(chain);
      chain = 0x0;
    }
  ...
  if (buffer->holes)
	  ...
	  (SEQ_LT (limit, hole->hole_end_seq - 1)))
	{
	  ...
	  fprintf (trimFile,
		   ...
		   buffer->holes->hole_end_seq,
		   isLimit ? '*' : ' ');
	  if (hole->hole_start->pep_m_next)
	    {
	      fprintf (trimFile,
		       "      first hole mbuff(%p) m_seq(%lu), len (%u), next(%p) m_seq(%lu), len (%u)\n",
		       hole->hole_start, hole->hole_start->m_seq, hole->hole_start->m_ext.len,
		       hole->hole_start->pep_m_next,
		       hole->hole_start->pep_m_next->m_seq, hole->hole_start->pep_m_next->m_ext.len);
	    }
#endif /* DEBUG_SNACK_OLD */
	  ...
	  while ((((int) (hole->length)) > 0) &&
		...
		{
		  hole->next_to_send = hole->hole_start->pep_m_next;
		}
	      hole->hole_start = hole->hole_start->pep_m_next;
	      if (hole->hole_start)
		{
...
if(!temp)
{
printf("Gateway-->mb_trim: deq_mbuff return NULL!\n");
fflush(stdout);
return timestamp;
}
...
mb_rtrim (struct _buffer *buffer, uint32_t limit)
{
  ...
  for (;;)
      ...
      if ((limit > 0) && (limit >= buffer->start->m_plen))
	{
	   ...
	   */
	  /* Dequeue and my_free the buffer */

	  limit -= buffer->start->m_plen;
	  temp = deq_mbuff (buffer);
if(!temp)
{
printf("Gateway-->mb_rtrim: deq_mbuff return NULL!\n");
fflush(stdout);
return;
}
	  temp->pep_m_next = 0x0;
...
int mb_merge_cti (struct mbuff *first, struct mbuff *second)
{
	...
	*/
	...
	if ((!(first && second)) || (first->m_seq > second->m_seq))
		return (-1);		/* Define an errno -  for use at "pseudo-kernel level" */
	...
	*/
	...
	for (; ((second) && (SEQ_GEQ ((first->m_seq + first->m_plen),second->m_seq)));)
    {
		/* 
		...
		*/
		...
		if ((first->pep_m_next == second) && (second->m_prev == first))		//在同一条链执行！！！！！！！！！
		{
			if (second->m_plen == 0) 
			{
				return(1);
			...
			if (first->m_seq == second->m_seq)
			{
				...
				printf("Gateway-->mb_merge: something wrong in m_seq!\n");
				
				if (first->m_plen >= second->m_plen)
				{
					/* Second is completely within first */						
					if ((first->pep_m_next = second->pep_m_next))
					{
						first->pep_m_next->m_prev = first;
						second->pep_m_next = NULL;
					}
			...
			if (SEQ_GEQ ((first->m_seq + first->m_plen), (second->m_seq + second->m_plen)))
			{
			...
			{
				overslop = (second->m_seq + second->m_plen) - (first->m_seq + first->m_plen);
				...
				first->parent->data_size -= (first->m_seq + first->m_plen) - second->m_seq;
		...
		*/
		...
		if (first->m_seq == second->m_seq)
		{
			if (first->m_plen >= second->m_plen)
			{
				...
				if (!(first->parent))
				{
					printf("Gateway-->mb_merge: first->parent1 is NULL!\n");

					if (!(second->parent))
					{
						printf("Gateway-->mb_merge: second->parent is NULL!\n");
			...
			else
			{
				...
				if (!(first->parent))
				{
					printf("Gateway-->mb_merge: first->parent2 is NULL!\n");
				...
				else
				{
					...
					/* Copy the links */
					if ((second->m_prev = first->m_prev))
					{
						second->m_prev->pep_m_next = second;
						first->m_prev = NULL;
					}

					if ((second->pep_m_next = first->pep_m_next))
					{
						second->pep_m_next->m_prev = second;
						first->pep_m_next = NULL;
					}

					overslop = second->m_plen - first->m_plen;
					second->parent->data_size += overslop;

					if (second->parent->last == first)
						second->parent->last = second;

					if (second->parent->start == first)
		...
		else
		{
			...
			*/
			...
			if (SEQ_GEQ ((first->m_seq + first->m_plen), (second->m_seq + second->m_plen)))
			{
				...
				if ((second->m_plen == 0) && (second->parent) && (second->pep_m_next == NULL) && (second->parent->last == second))
				{
				...
				if (!(first->parent))
				{
					printf("Gateway-->mb_merge: first->parent3 is NULL!\n");
			...
			else
			{
				if (!(first->parent))
				{
					printf("Gateway-->mb_merge: first->parent4 is NULL!\n");
					...
					first->parent->data_size += (second->m_seq - first->m_seq);
					...
					if (second->pep_m_next)
					{
						first->pep_m_next = second->pep_m_next;
						first->pep_m_next->m_prev = first;
						second->pep_m_next = NULL;
					}
...
mb_merge (struct mbuff *first, struct mbuff *second)
{
  ...
  for (; ((second) && (SEQ_GEQ ((first->m_seq + first->m_plen),
				second->m_seq)));)
    {
      ...
      if ((first->pep_m_next == second) && (second->m_prev == first))
	{
	  /* 
	  ...
	  first->m_ext.len += (second->m_plen - overslop);
		if(!(first->parent))
		{
			printf("Gateway-->Function mb_merge: firet->parent is NULL!\n");
			fflush(stdout);
			return -1;
		}
	  first->parent->data_size -= overslop;
	  if ((first->pep_m_next = second->pep_m_next))
	    second->pep_m_next->m_prev = first;
...
printf("Gateway-->mb_merge: second->parent is NULL!\n");
      ...
      if (overslop < 0)		/* Error! */
	{
      ...
      /* 
       * Now, let's make sure we can safely my_free the second: 
       * Make sure we don't my_free the new stuff;
...
if(buffer->write_off < 0)
{
	printf("Write_off Error: File %s, Line %d, write_off = %d!\n", __FILE__, __LINE__, buffer->write_off);
}
...
mb_attach (struct mbuff *mbuffer, char *linear_space, int space_len)
{
  ...
  if ((!(length)) || !(new_head = alloc_mbclus (0)))
	{
printf("Gateway-->Function mb_attach: mem leak---1!\n");
		fflush(stdout);
  ...
  while (length > 0)
    {
      if (!(new_element = alloc_mbclus (0)))
{
printf("Gateway-->Function mb_attach: mem leak---2!\n");
		fflush(stdout);
