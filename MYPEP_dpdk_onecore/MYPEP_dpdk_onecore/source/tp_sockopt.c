	...
	switch (optname)
	  ...
	  case SCPS_SO_RCVBUF:
	      ...
	      if (optlen == sizeof (uint32_t))
		{
		   ...
		   * size.
		   */
		  if ((((tp_Socket *) s)->state < tp_StateSYNSENT) && (((tp_Socket *) s)->sockFlags & TF_REQ_SCALE))
		    {
		      ((tp_Socket *) s)->request_r_scale = 0;

		      while ((((tp_Socket *) s)->request_r_scale < TP_MAX_WINSHIFT) && 
							((TP_MAXWIN << ((tp_Socket *)s)->request_r_scale) < ((tp_Socket *) s)->app_rbuff->max_size))
					((tp_Socket *) s)->request_r_scale++;

				//((tp_Socket *) s)->request_r_scale = 9;
		    }
#endif /* OPT_SCALE */
		  return (0);
		}
	  ...
	  case SCPS_RATE:
	      ...
	      if (optlen == sizeof (uint32_t))
		{
		  ...
		  ((tp_Socket *) s)->rt_route->current_credit =
		    max ((int)(((tp_Socket *) s)->rt_route->MTU * RATE_BUCKET_FACTOR),
			 (int)(((tp_Socket *) s)->rt_route->bytes_per_interval) );
			printf("-----------MTU:%d, current_credit %d------------\n", ((tp_Socket *) s)->rt_route->MTU, ((tp_Socket *) s)->rt_route->current_credit);
