...

{
	...
	new_rt = (route *) my_malloc (sizeof (route));

	if(!new_rt)
	{
		printf("Gateway-->Function route_create_default: my_malloc return NULL!\n");
		fflush(stdout);
		return -1;
	}
...
int cong_control;
{
	...
	if(!new_rt)
	{
		printf("Gateway-->Function route_add: my_malloc return NULL!\n");
		fflush(stdout);
		return -1;
	}
	memset (new_rt, 0, sizeof (route));
...
#ifndef XXXXX
{
	...
	add_route_for_other_proto(new_rt);
...
int route_id;
{
	...
	del_route_for_other_proto(route_sock_id);
...
#ifdef GATEWAY_ROUTER
int route_rt_delete (this_route)
route *this_route;
{
	route *tmp_this = NULL;
	route *tmp_prev = NULL;
