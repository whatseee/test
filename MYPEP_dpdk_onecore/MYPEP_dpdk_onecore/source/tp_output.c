  ...
  if (s->flags & tp_FlagSYN)
      ...
      if (s->capabilities & CAP_JUMBO)
	{
	  ...
	  opt[1] = 4;		/* We *always* are 4 bytes in length; */
			...
			/****************************** add CETC54 Sending Speed Notification option******************************/
			opt[optlen]		= TPOPT_NOP;
			opt[optlen+1]	= TPOPT_NOP;
			opt[optlen+2]	= TPOPT_SCPS;  //\B1\EA־
			opt[optlen+3]	= 6;				//\B3\A4\B6\C8
			opt[optlen+4]	= 0x54;			//ID
			opt[optlen+5]	= 0x11;			//\B8\DF4bit:\C4\DA\C8ݳ\A4\B6\C8(˫\D7ֽ\DAΪ\B5\A5λ);  \B5\CD4bit:\C4\DA\C8ݱ\EA־ 01\A3\ADCETC54 Sending Speed Notification
			*((unsigned short*)(opt+optlen+6)) = htons( (s->rt_route->rate/8) / 1000 );
			optlen += 8;
			//printf("tp_BuildHdr: s->rt_route->rate=%d!!!\n", s->rt_route->rate);
			/****************************** add CETC54 Sending Speed Notification option******************************/
	}
#endif /* OPT_SCPS */
#ifdef OPT_SCPS 
...
/*  if (s->capabilities & CAP_SNACK)
	...
	{
		printf("th_x2:%x, th_off:%x\n", th->th_x2, th->th_off);
		printf("Gateway-->Function tp_BuildHdr: memcpy(option_start, opt, optlen) optlen=%d!\n", optlen);
		fflush(stdout);
	}
...
struct mbuff *
tp_BuildHdr_KA (tp_Socket * s, struct mbuff *mbuffer, int push)
{
  tp_Header *th;
  uint32_t *option_start;
  short offset;
  ...
  if (long_temp) {
    th->seqnum = htonl (s->seqsent -1);
  } else {
    th->seqnum = htonl (s->seqnum - 1);
  }
  tp_WinAck (s, th);
  /* if (s->otimers[Del_Ack]->set)
     clear_timer(s->otimers[Del_Ack], 1); */
...
tp_next_to_send (tp_Socket * s, struct _hole_element **hole)
{
  ...
  if (local_hole)
      ...
      if (local_hole->Embargo_Time)
	{
      ...
      if ((local_hole->hole_start)&&(local_hole->hole_start->m_mfx_count))
	{
...
//                   && (s->send_buff->holes)
			      ...
			      s->send_buff->holes->hole_start_seq))
		{
		  ...
		  if ((!(hole->next_to_send)) ||
		    ...
		    hole->next_to_send = hole->next_to_send->pep_m_next;
  ...
  while ((limit_ok > 3) && ((mbuffer) || (s->hole_start != s->hole_end)
			    || (s->send_buff->send)))
    {
      ...
      min_limit = min (s->rt_route->max_burst_bytes,
		       min (s->rt_route->current_credit, (int)(s->snd_cwnd)));
...
uint32_t tp_iovCoalesce (tp_Socket * s, struct mbuff * m, uint32_t * bytes_sent)
{
	...
	if( (!s) || (!m) || (!bytes_sent) )
	{
		printf("Gateway-->Function tp_iovCoalesce: s=%d, m=%d, bytes_sent=%d!\n", (int)s, (int)m, (int)bytes_sent);
		fflush(stdout);
		return 0;
	}

	//int i=0,k=0,symbol=0;            //added by penghua
	//struct _interface *interface;    //added by penghua
	...
	*/
	...
	if (s->capabilities & CAP_COMPRESS)
    {
		...
		*/
		...
		if (((th->flags & tp_FlagSYN) == 0) && ((s->sockFlags & (TF_COMPRESSING)) == (TF_COMPRESSING)))
		{
			/* working_mbuff = clone_mbuff(m); *//* For testing ONLY */
			copy_mbuff (s->scratch_buff, m);
			working_mbuff = s->scratch_buff;
			chl = tp_Compress (s, m, (working_mbuff->m_pktdat + s->th_off));
...
/*
	...
	if(s->recv_tap_if == interface->tap_b_fd) //\BD\ABҪ\B4\D3BIF\B7\A2\B3\F6\B5İ\FC
	{
		for(i=0; i<PROTO_MAX; i++)
		{
			for(k = 0; k < MAX_MCPC_DST_STA; k++)
			{
				if(other_proto[i][1].r_and_q[k].sign == 1)
				{
					if(other_proto[i][1].r_and_q[k].rt->route_sock_id == s->rt_route->route_sock_id)
					{
						if(!q_empty (other_proto[i][1].r_and_q[k].q))  //\C8\E7\B9\FB\B6\D3\C1в\BB\BFգ\ACsymbol\D6\C31
						{
							symbol = 1;
							break;
						}
...
#ifdef GATEWAY
/*
#ifdef TAP_INTERFACE   //added by penghua
	if ( (!(s->gateway_flags & GATEWAY_SCPS_TP_SESSION)) || ((s->gateway_flags & GATEWAY_SCPS_TP_SESSION) && ((symbol==0) && (s->rt_route->current_credit >= long_temp) &&	(bytes_sent) && (s->rt_route->max_burst_bytes >= long_temp))))
#else
	if ( (!(s->gateway_flags & GATEWAY_SCPS_TP_SESSION)) || ((s->gateway_flags & GATEWAY_SCPS_TP_SESSION) && ((s->rt_route->current_credit >= long_temp) &&	(bytes_sent) && (s->rt_route->max_burst_bytes >= long_temp))))
#endif
*/
	if ( (!(s->gateway_flags & GATEWAY_SCPS_TP_SESSION)) || ((s->gateway_flags & GATEWAY_SCPS_TP_SESSION) && ((s->rt_route->current_credit >= (int)long_temp) &&	(bytes_sent) && (s->rt_route->max_burst_bytes >= long_temp))))
#else /* GATEWAY */
	if ((s->rt_route->current_credit >= (int)long_temp) && (bytes_sent) && (s->rt_route->max_burst_bytes >= long_temp))
#endif /* GATEWAY */
    {
		...
		if (s->rt_route->encrypt_ipsec) 
		{
			...
			{
				bytes_to_be_credited = s->rt_route->encrypt_pre_overhead + long_temp + s->rt_route->encrypt_post_overhead;
			}
			else
			{
				bytes_to_be_credited = s->rt_route->encrypt_pre_overhead + long_temp + (s->rt_route->encrypt_block_size - ((long_temp + s->rt_route->encrypt_pre_overhead) % s->rt_route-> encrypt_block_size)) + s->rt_route->encrypt_post_overhead;
			}
		...
		*bytes_sent = long_temp;
		...
		{
			if (s->rt_route->current_credit - (int)long_temp > s->rt_route->current_credit) 
			{
