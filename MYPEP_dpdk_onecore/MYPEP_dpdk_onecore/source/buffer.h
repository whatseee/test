...
struct _sys_memory
  {
    ...
    uint32_t clust_created;	/* Number of clusters actually my_malloc'd    */
...
#define pep_m_next            m_hdr.mh_next
...
/* mbuff types */
#define MT_FREE           0	/* should be on my_free list      */
...
int
  mb_merge_cti (struct mbuff *first, struct mbuff *second);
