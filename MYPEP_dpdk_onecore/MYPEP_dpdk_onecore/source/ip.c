...
ip_trequest (tp_Socket * s, struct mbuff *m, uint32_t * bytes_sent)
{
  ...
  nroute = s->rt_route;
      ...
      memcpy (&(s->np_rqts.src_mac_addr [0]), &(s->src_mac_addr [0]), 6);
		...
		s->np_rqts.TPID = s->TPID;
		s->np_rqts.TCI = s->TCI;
      s->np_rqts.frame_type = s->frame_type;
	s->np_rqts.mpls_depth = s->mpls_depth_s;
	//printf("s->np_rqts.frame_type 0x%x\n", s->np_rqts.frame_type);
	//printf("s->np_rqts.mpls_depth %d\n", s->np_rqts.mpls_depth);
	//fflush(stdout);
		for(local_cnt=0; local_cnt<s->mpls_depth_s; local_cnt++)
		{
			s->np_rqts.mpls[local_cnt] = s->mpls_s[local_cnt];
		}	
...
*/
	if(interface == sock_interface)
	{
		...
		special_ip_addr = interface->remote_ipaddr;    //temporarily
	}	 
#endif /* GATEWAY_DUAL_INTERFACE */
...
//#ifdef TAP_INTERFACE
	int rc;
	unsigned char proto;
	unsigned int  ip_header_length = 0;   //added by penghua
	tp_Header *tp = NULL;					//added by penghua
//#endif /* TAP_INTERFACE */
...
//pthread_mutex_lock(&mutex);
	...
	if ((interface && (cc = ll_nbreceive (interface, &in_data, MAX_MTU, &offset)) > 0))
    	{
        	...
        	if (inv4_GetProtocol (ip) == SCPSUDP) 
		{
			...
			{ 
				if (gw_ifs.aif_layering == GATEWAY_LAYERING_UDP) 
				{
					...
					in_data -> special_ip_addr = (ip->nl_head.ipv4.source);
				}
		...
		if ((ntohs (ip->nl_head.ipv4.vht) & 0xf000) != 0x4000)   //\C8\E7\B9\FB\B2\BB\CA\C7IPV4
		{
...
//#ifdef TAP_INTERFACE
		...
		if( proto == SCPSTP )
		{
			ip_header_length = ((ntohs (ip->nl_head.ipv4.vht)) & 0x0f00) >> 6;
			tp = (tp_Header *) ((void *) ip + ip_header_length);

			if(-1 != filter_rule_find_match_rule(ntohl(ip->nl_head.ipv4.destination), ntohs(tp->dstPort), ntohl(ip->nl_head.ipv4.source), ntohs(tp->srcPort)) )
			{
	...
	//      case SCPSUDP:
			...
			default:
				other_proto_Handler (interface, &in_data, MAX_MTU, &offset, rqts, proto);
				//printf("ip.c 543: interface->incoming.size= %d\n",interface->incoming.size);
				return (-1);
				break;
		}
//#endif /* TAP_INTERFACE */
		...
		if( (ip->nl_head.ipv4.checksum == 0) || ((ip_cks = checksum ((word *) ip, inv4_GetHdrlenBytes (ip))) == 0xFFFF) )
		{
