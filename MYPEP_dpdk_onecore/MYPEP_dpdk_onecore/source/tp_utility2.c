...
void tp_dooption_ecbs_cetc54(s, cp, opt_index, buf_size)
tp_Socket *s;
unsigned char *cp;
...
unsigned short *buf_size;
{
	*buf_size = 0;
	...
	if(cp[*opt_index] == 0x54)
	{
		(*opt_index) = (*opt_index) + 1;

		if(cp[*opt_index] == 0x11)
		{
			(*opt_index) = (*opt_index) + 1;
			(*buf_size)  = ntohs( *((unsigned short*)(&cp[*opt_index])) );
		}
	}
}
			...
			case TPOPT_SCPS:
				...
				else 
				{
          		/* This is the Extended Capability Binding Space Logic */
					...
					tp_dooption_ecbs_cetc54(s, cp, &opt_index, &buf_size1);
					*buf_size = buf_size1;
...
*/
			...
			case TPOPT_WINDOW:
	  			...
	  			s->requested_s_scale = (cp[2] < TP_MAX_WINSHIFT) ? cp[2] : TP_MAX_WINSHIFT;
	  			break;
			...
			case TPOPT_SNACK1:
	  			...
	  			s->SNACK1_Send_Offset = ntohs (offset) * (s->my_mss_offer - TP_HDR_LEN);
	   		...
	   		* ptr will provide the appropriate handle for retransmission
	      		...
	      		for (; ((snack_send_offset > 0) && (ptr)); ptr = ptr->pep_m_next)
	      		...
	      		SCRUB_HOLES (s);
	       		s->send_buff->holes = add_hole (s->send_buff->holes, ptr, s->SNACK1_Send_Hole, tp_now, 0, s->max_seqsent,
			      											s->snack_delay);
	      		VERIFY_BUFFER_HOLES (s->send_buff);
	      		SCRUB_HOLES (s);
	    			}
